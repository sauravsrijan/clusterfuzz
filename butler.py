...
def _setup_args_for_remote(parser):
    ...
    parser_tailf = subparsers.add_parser(
        "tailf",
        help=(
            "Print the last lines of logs and wait for more. "
    ...
    parser_rdp = subparsers.add_parser(
        "rdp",
        help=(
            "Launch Remmina with correct configuration (e.g. IP address for the"
            " instance)."
        ),
    )
    parser_rdp.add_argument(
        "--share-path",
        help=(
            "The share path that is mounted on the remote instance."
...
def main():
    ...
    parser_go_unittest = subparsers.add_parser("go_unittest", help="Run Go unit tests.")
    ...
    parser_js_unittest.add_argument(
        ...
        "--persist",
        action="store_true",
        help=("Do not close browser when tests " "finish. Good for debugging."),
    )
    ...
    parser_deploy.add_argument(
        "-f", "--force", action="store_true", help="Force deploy from any branch."
    )
    parser_deploy.add_argument(
        "-c", "--config-dir", required=True, help="Path to application config."
    )
    ...
    parser_deploy.add_argument("--targets", nargs="*", default=["appengine", "zips"])
    parser_deploy.add_argument(
    ...
    parser_run_server.add_argument(
        "-b", "--bootstrap", action="store_true", help="Bootstrap the local database."
    )
    parser_run_server.add_argument(
        "--storage-path",
    ...
    parser_run_server.add_argument(
        ...
        action="store_true",
        help=(
            "Don't install dependencies before running this command (useful "
            "when you're restarting the server often)."
        ),
    )
    parser_run_server.add_argument("--log-level", default="info", help="Logging level")
    parser_run_server.add_argument(
    ...
    parser_run.add_argument(
        "script_name", help="The script module name under `./local/butler/scripts`."
    )
    parser_run.add_argument(
        "--non-dry-run",
        action="store_true",
    ...
    parser_run_bot.add_argument("--name", default="test-bot", help="Name of the bot.")
    parser_run_bot.add_argument(
        "--server-storage-path", default="local/storage", help="Server storage path."
    )
    ...
    parser_create_config.add_argument(
        "--gce-zone", type=str, default="us-central1-f", help="Region for GCE VMs."
    )
    parser_create_config.add_argument(
        "--appengine-location",
        type=str,
