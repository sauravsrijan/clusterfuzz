...
def _read_json(url):
    ...
    try:
        ...
        logs.log_warn("Empty or malformed code coverage JSON (%s): %s." % (url, str(e)))
...
def _coverage_information(summary_path, name, report_info):
    ...
    date = datetime.datetime.strptime(
        report_info["report_date"], data_types.COVERAGE_INFORMATION_DATE_FORMAT
    ).date()

    # |name| can be either a project qualified fuzz target name or a project name.
    cov_info = data_handler.get_coverage_information(name, date, create_if_needed=True)
...
def _process_fuzzer_stats(fuzzer, project_info, project_name, bucket):
    ...
    logs.log("Processing fuzzer stats for %s (%s)." % (fuzzer_name, fuzzer_info_path))
...
def _process_project_stats(project_info, project_name):
    ...
    logs.log(
        "Processing total stats for %s project (%s)." % (project_name, summary_path)
    )
...
def _process_project(project, bucket):
    ...
    entities = []
    for fuzzer in storage.list_blobs(report_info["fuzzer_stats_dir"], recursive=False):
        entities.append(
            _process_fuzzer_stats(fuzzer, report_info, project_name, bucket)
        )

    logs.log(
        "Processed coverage for %d targets in %s project."
