...
SpecificationMatch = collections.namedtuple(
    "SpecificationMatch", ["new_weight", "reason"]
)
...
OOM_SPECIFICATION = QuerySpecification(
    query_format=GENERIC_QUERY_FORMAT.format(field_name="oom_count", min_weight=0.25),
    formatter=_past_day_formatter,
...
CRASH_SPECIFICATION = QuerySpecification(
    query_format=GENERIC_QUERY_FORMAT.format(field_name="crash_count", min_weight=0.50),
    formatter=_past_day_formatter,
...
NEW_FUZZER_SPECIFICATION = QuerySpecification(
    query_format=NEW_FUZZER_FORMAT, formatter=_new_fuzzer_formatter, reason="new fuzzer"
)
...
def update_weight_for_target(fuzz_target_name, job, match):
    ...
    logs.log(
        "Adjusted weight to %f for target %s and job %s (%s)."
...
def update_matches_for_specification(specification, client, engine, matches, run_set):
    """Run a query and adjust weights based on a given query specification."""
    query = specification.formatter(
        specification.query_format, fuzzer_stats.dataset_name(engine)
    )
...
def update_target_weights_for_engine(client, engine, specifications):
    ...
    target_jobs = data_types.FuzzTargetJob.query(
        data_types.FuzzTarget.engine == engine
    ).filter(data_types.FuzzTargetJob.weight != 1.0)

    for target_job in target_jobs:
        matches[(target_job.fuzz_target_name, target_job.job)] = RESTORE_DEFAULT_MATCH
...
class Handler(base_handler.Handler):
    ...
    def get(self):
        ...
        update_target_weights_for_engine(client, "libFuzzer", LIBFUZZER_SPECIFICATIONS)
