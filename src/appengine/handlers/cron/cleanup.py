...
def get_jobs_and_platforms_for_top_crashes():
  ...
  for job in all_jobs:
    ...
    if utils.string_is_true(job_environment.get(
        "CUSTOM_BINARY")) or job_environment.get("SYSTEM_BINARY_DIR"):
      continue
...
def mark_issue_as_closed_if_testcase_is_fixed(policy, testcase, issue):
  ...
  if issue_tracker_utils.was_label_added(
      issue, verified_label) or issue_tracker_utils.was_label_added(
          issue, policy.label("wrong")):
    return
...
def notify_closed_issue_if_testcase_is_open(policy, testcase, issue):
  ...
  if issue.status in [policy.status("fixed"), policy.status("verified")]:
    issue_comment = (
        ...
        "fix was incorrect or incomplete, please re-open the bug.").format(
            id=testcase.key.id())
...
def _update_issue_when_uploaded_testcase_is_processed(policy, testcase, issue,
                                                      description,
                                                      update_bug_summary,
                                                      notify):
...
def update_os_labels(policy, testcase, issue):
  ...
  logs.log(
      "Found %d platforms for the testcase %d." %
...
def update_fuzz_blocker_label(policy, testcase, issue,
  ...
  fuzzer_name = testcase.get_metadata(
      "fuzzer_binary_name") or testcase.fuzzer_name
  update_message = (
