...
LIBFUZZER_ASAN_JOB = JobInfo(
    "libfuzzer_asan_", "libfuzzer", "address", ["libfuzzer", "engine_asan", "prune"]
)
LIBFUZZER_MSAN_JOB = JobInfo(
    "libfuzzer_msan_", "libfuzzer", "memory", ["libfuzzer", "engine_msan"]
)
LIBFUZZER_UBSAN_JOB = JobInfo(
    "libfuzzer_ubsan_", "libfuzzer", "undefined", ["libfuzzer", "engine_ubsan"]
)
LIBFUZZER_ASAN_I386_JOB = JobInfo(
    "libfuzzer_asan_i386_",
    "libfuzzer",
    "address",
    ["libfuzzer", "engine_asan"],
    architecture="i386",
)
...
JOB_MAP = {
    "libfuzzer": {
        ...
        "i386": {"address": LIBFUZZER_ASAN_I386_JOB,},
    },
    "afl": {"x86_64": {"address": AFL_ASAN_JOB,}},
    "honggfuzz": {"x86_64": {"address": HONGGFUZZ_ASAN_JOB,},},
    "none": {"x86_64": {"address": NO_ENGINE_ASAN_JOB,}},
}
...
def get_github_url(url):
    ...
    response = requests.get(
        url, params={"client_id": client_id, "client_secret": client_secret}
    )
...
def get_oss_fuzz_projects():
    ...
    ossfuzz_tree_url = (
        "https://api.github.com/repos/google/oss-fuzz/" "git/trees/master"
    )
...
def get_jobs_for_project(project, info):
    ...
    sanitizers = _process_sanitizers_field(info.get("sanitizers", DEFAULT_SANITIZERS))
    if not sanitizers:
    ...
    for engine in engines:
        ...
        for architecture in architectures:
            ...
            for sanitizer, options in six.iteritems(sanitizers):
                experimental = options.get("experimental", False) or info.get(
                    "experimental", False
                )
...
def _add_users_to_bucket(info, client, bucket_name, iam_policy):
    ...
    if binding:
        ...
        filtered_members = [member for member in binding["members"] if member in ccs]

        if len(filtered_members) != len(binding["members"]):
            # Remove old members.
            binding["members"] = filtered_members
            iam_policy = storage.set_bucket_iam_policy(client, bucket_name, iam_policy)

    # We might have no binding either from start or after filtering members above.
    # Create a new one in those cases.
    binding = storage.get_or_create_bucket_iam_binding(
...
def _set_bucket_service_account(service_account, client, bucket_name, iam_policy):
    """Set service account for a bucket."""
    # Add service account as objectAdmin.
    binding = storage.get_or_create_bucket_iam_binding(
        iam_policy, OBJECT_ADMIN_IAM_ROLE
    )
...
def cleanup_pubsub_topics(project_names):
    ...
    for topic in client.list_topics(pubsub.project_name(application_id)):
        ...
        if not name.startswith(tasks.JOBS_PREFIX) and not name.startswith(
            tasks.HIGH_END_JOBS_PREFIX
        ):
...
class ProjectSetup(object):
    ...
    def _create_service_accounts_and_buckets(self, project, info):
        ...
        storage.create_bucket_if_needed(quarantine_bucket_name, QUARANTINE_LIFECYCLE)
        ...
        add_service_account_to_bucket(
            client,
            self._deployment_bucket_name(),
            service_account,
        ...
        for data_bundle in data_bundles:
            ...
            add_service_account_to_bucket(
                client, data_bundle_bucket_name, service_account, OBJECT_VIEWER_IAM_ROLE
            )
    ...
    def _get_build_bucket_path(
        self, project_name, info, engine, memory_tool, architecture
    ):
        ...
        for template in get_jobs_for_project(project, info):
            ...
            if self._add_revision_mappings:
                revision_vars_url = self._revision_url_template.format(
                    project=project,
                    bucket=self._get_build_bucket(
                        template.engine, template.architecture
                    ),
            ...
            if (
                template.engine == "libfuzzer"
                and template.architecture == "x86_64"
                and "dataflow" in info.get("fuzzing_engines", DEFAULT_ENGINES)
            ):
            ...
            if self._additional_vars:
                ...
                for key, value in sorted(six.iteritems(additional_vars)):
                    job.environment_string += "{} = {}\n".format(
                        key, str(value).encode("unicode-escape").decode("utf-8")
                    )
    ...
    def set_up(self, projects):
        ...
        enabled_projects = [
            project for project, info in projects if not info.get("disabled", False)
        ]
...
class Handler(base_handler.Handler):
    ...
    def get(self):
        ...
        project_setup_config = local_config.ProjectConfig().sub_config("project_setup")
        ...
        config = ProjectSetup(
            ...
            add_info_labels=project_setup_config.get("add_info_labels", default=False),
