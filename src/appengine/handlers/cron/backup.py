...
class Handler(base_handler.Handler):
    ...
    def get(self):
        """Handle a cron job."""
        backup_bucket = local_config.Config(local_config.PROJECT_PATH).get(
            "backup.bucket"
        )
        ...
        kinds = [
            kind
            for kind in ndb.Model._kind_map  # pylint: disable=protected-access
            if (not kind.startswith("_") and kind not in EXCLUDED_MODELS)
        ]

        app_id = utils.get_application_id()
        timestamp = datetime.datetime.utcnow().strftime("%Y-%m-%d-%H:%M:%S")
        output_url_prefix = "gs://{backup_bucket}/datastore-backups/{timestamp}".format(
