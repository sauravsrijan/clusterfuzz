...
def _is_privileged_user(email):
    ...
    privileged_user_emails = (
        db_config.get_value("privileged_users") or ""
    ).splitlines()
    return any(
...
def _is_blacklisted_user(email):
    """Check if an email is in the privileged users list."""
    blacklisted_user_emails = (
        db_config.get_value("blacklisted_users") or ""
    ).splitlines()
    return any(
...
def get_access(need_privileged_access=False, job_type=None, fuzzer_name=None):
    ...
    if fuzzer_name and external_users.is_fuzzer_allowed_for_user(email, fuzzer_name):
        return UserAccess.Allowed
...
def can_user_access_testcase(testcase):
    ...
    relaxed_restrictions = config.relax_testcase_restrictions or _is_domain_allowed(
        user_email
    )
