...
def _do_operation_with_retries(operation, project, zone, wait_for_completion):
    ...
    for _ in range(NUM_RETRIES + 1):
        try:
            ...
            logs.log_error(
                "Failed to wait for Compute Engine operation. "
...
def _do_instance_operation(
    operation, instance_name, project, zone, wait_for_completion
):
...
def _execute_api_call_with_retries(api_func):
    ...
    if last_exception is not None:
        ...
        logs.log_error(
            'Compute engine API call "%s" failed with exception:\n%s'
            % (uri, last_exception)
        )
        return None
...
def add_metadata(instance_name, project, zone, key, value, wait_for_completion):
    ...
    operation = api.instances().setMetadata(
        body={"fingerprint": fingerprint, "items": existing_metadata,},
        instance=instance_name,
        project=project,
        zone=zone,
    )

    return _do_operation_with_retries(operation, project, zone, wait_for_completion)


def create_disk(
    disk_name, source_image, size_gb, project, zone, wait_for_completion=False
):
    """Create a disk."""
    api = _get_api()
    operation = api.disks().insert(
        body={"name": disk_name, "sizeGb": size_gb,},
...
def recreate_instance_with_disks(
    instance_name, project, zone, additional_metadata=None, wait_for_completion=False
):
    ...
    if not instance_info or "disks" not in instance_info or not instance_info["disks"]:
        logs.log_error(
    ...
    for disk in disks:
        ...
        if not create_disk(
            disk_name, source_image, size_gb, project, zone, wait_for_completion=True
        ):
    ...
    operation = api.instances().insert(body=instance_info, project=project, zone=zone)
...
def remove_metadata(instance_name, project, zone, key, wait_for_completion):
    ...
    api = _get_api()
    operation = api.instances().setMetadata(
        body={"fingerprint": fingerprint, "items": filtered_metadata,},
        instance=instance_name,
        project=project,
        zone=zone,
    )
...
def reset_instance(instance_name, project, zone, wait_for_completion=False):
    """Reset an instance."""
    return _do_instance_operation(
        "reset", instance_name, project, zone, wait_for_completion
    )
