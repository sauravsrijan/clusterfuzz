...
def copy_file_to(local_file_path_or_handle,
  ...
  if isinstance(local_file_path_or_handle,
                basestring) and not os.path.exists(local_file_path_or_handle):
...
def store_file_in_cache(file_path,
  ...
  if not os.path.exists(file_path):
    logs.log_error("Local file %s does not exist, nothing to store in cache." %
                   file_path)
    return
  ...
  if not locks.acquire_lock(
    ...
    logs.log_warn("Unable to fetch lock to update cache file %s, skipping." %
                  filename)
    return
