...
class ReceivedMessage(Message):
    ...
    def __init__(
        self, client, subscription, data, attributes, message_id, publish_time, ack_id
    ):
...
class PubSubClient(object):
    ...
    def modify_ack_deadline(self, subscription, ack_ids, seconds):
        ...
        request = (
            ...
            .projects()
            .subscriptions()
            .modifyAckDeadline(subscription=subscription, body=body)
        )
        self._execute_with_retry(request)

    def publish(self, topic, messages):
        """Publish a message to a topic."""
        request_body = {"messages": [_message_to_dict(message) for message in messages]}
        request = (
            self._api_client()
            .projects()
            .topics()
            .publish(topic=topic, body=request_body)
        )
    ...
    def pull_from_subscription(
        self, subscription, max_messages=_DEFAULT_MAX_MESSAGES, acknowledge=False
    ):
        ...
        request = (
            ...
            .projects()
            .subscriptions()
            .pull(subscription=subscription, body=request_body)
        )
        response = self._execute_with_retry(request)
        if response is None:
    ...
    def create_topic(self, name, labels=None):
        ...
        request = self._api_client().projects().topics().create(name=name, body=body)
        self._execute_with_retry(request)
    ...
    def create_subscription(
        self, name, topic, ack_deadline=MAX_ACK_DEADLINE, labels=None
    ):
        ...
        request = (
            self._api_client().projects().subscriptions().create(name=name, body=body)
        )
        self._execute_with_retry(request)
    ...
    def delete_subscription(self, name):
        ...
        request = (
            self._api_client().projects().subscriptions().delete(subscription=name)
        )
        self._execute_with_retry(request)

    def get_subscription(self, name):
        """Get a subscription, or None if it does not exist."""
        request = self._api_client().projects().subscriptions().get(subscription=name)
        response = self._execute_with_retry(request)
        if not response:
            return None
    ...
    def list_topics(self, project, page_size=1000):
        ...
        request = (
            ...
            .projects()
            .topics()
            .list(project=project, pageSize=page_size)
        )
        response = self._execute_with_retry(request)
        ...
        while True:
            ...
            request = (
                ...
                .projects()
                .topics()
                .list(project=project, pageToken=next_page_token, pageSize=page_size)
            )
            response = self._execute_with_retry(request)
    ...
    def list_topic_subscriptions(self, topic, page_size=1000):
        ...
        request = (
            ...
            .topics()
            .subscriptions()
            .list(topic=topic, pageSize=page_size)
        )
        response = self._execute_with_retry(request)
        ...
        while True:
            ...
            request = (
                ...
                .topics()
                .subscriptions()
                .list(topic=topic, pageToken=next_page_token, pageSize=page_size)
            )
            response = self._execute_with_retry(request)
...
def subscription_name(project, name):
    ...
    return "projects/{project}/subscriptions/{name}".format(project=project, name=name)
...
def _raw_message_to_message(client, subscription, raw_message_response):
    ...
    data = base64.b64decode(raw_message["data"]) if "data" in raw_message else None
