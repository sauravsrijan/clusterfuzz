...
def find_windows_for_process(process_id):
  ...
  visible_windows = []
  for pid in pids:
    windows = shell.execute_command("%s search --all --pid %d --onlyvisible" %
                                    (xdotool_path, pid))
...
def get_random_gestures(gesture_count):
  ...
  for _ in range(gesture_count):
    ...
    if random_gesture == "drag":
      ...
      random_gesture = random_gesture.replace("Function",
  ...
  for window in windows:
    ...
    for gesture in gestures:
      ...
      if gesture_type == "windowsize":
        shell.execute_command("%s %s %d %s" %
                              (xdotool_path, gesture_type, window, gesture_cmd))
      else:
        shell.execute_command("%s %s -- %s" %
                              (xdotool_path, gesture_type, gesture_cmd))
