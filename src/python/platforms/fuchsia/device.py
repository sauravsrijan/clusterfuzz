...
def _fetch_qemu_vars():
    ...
    * build/buildtools/linux-x64/clang/bin/llvm-symbolizer
    ...
    qemu_vars["kernel_path"] = os.path.join(
        fuchsia_resources_dir, "target", "x64", "multiboot.bin"
    )
    ...
    qemu_vars["drive_path"] = os.path.join(
        fuchsia_resources_dir, "target", "x64", "fvm.blk"
    )
    os.chmod(qemu_vars["drive_path"], 0o644)
    qemu_vars["fuchsia_zbi"] = os.path.join(
        fuchsia_resources_dir, "target", "x64", "fuchsia.zbi"
    )
    qemu_vars["initrd_path"] = os.path.join(fuchsia_resources_dir, "fuchsia-ssh.zbi")
    return qemu_vars
...
class QemuProcess(object):
    ...
    def __init__(self):
        ...
        if not QemuProcess.LOG_PATH:
            QemuProcess.LOG_PATH = os.path.join(
                tempfile.gettempdir(), "fuchsia-qemu-log"
            )
    ...
    def create(self):
        ...
        environment.set_value(
            "FUCHSIA_RESOURCES_DIR", qemu_vars["fuchsia_resources_dir"]
        )
        ...
        host = Host.from_dir(
            os.path.join(qemu_vars["fuchsia_resources_dir"], "build", "out", "default")
        )
...
def extend_fvm(fuchsia_resources_dir, drive_path):
    ...
    fvm_tool_path = os.path.join(
        fuchsia_resources_dir, "build", "out", "default.zircon", "tools", "fvm"
    )
...
def add_keys_to_zbi(fuchsia_resources_dir, initrd_path, fuchsia_zbi):
    ...
    zbi_tool = os.path.join(
        fuchsia_resources_dir, "build", "out", "default.zircon", "tools", "zbi"
    )
    os.chmod(zbi_tool, 0o500)
    authorized_keys_path = os.path.join(
        fuchsia_resources_dir, ".ssh", "authorized_keys"
    )
    process = new_process.ProcessRunner(
        zbi_tool,
        [
    ...
    result = process.run_and_wait()
    if result.return_code or result.timed_out:
        raise errors.FuchsiaSdkError(
            "Failed to add keys to Fuchsia ZBI: " + result.output
        )
