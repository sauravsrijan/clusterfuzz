...
def copy_remote_file_to_local(remote_file_path, local_file_path):
    ...
    shell.create_directory(os.path.dirname(local_file_path), create_intermediates=True)
...
def factory_reset():
    ...
    run_as_root()
    run_shell_command(["am", "broadcast", "-a", "android.intent.action.MASTER_CLEAR"])
...
def file_exists(file_path):
    ...
    expected = "0"
    result = run_shell_command("'test -f \"%s\"; echo $?'" % file_path, log_error=False)
    return result == expected
...
def get_fastboot_path():
    ...
    return os.path.join(environment.get_platform_resources_directory(), "fastboot")
...
def get_process_and_child_pids(process_name):
    ...
    while True:
        ...
        for line in ps_output_lines:
            ...
            if (
                process_name in line_process_name
                or MONKEY_PROCESS_NAME in line_process_name
            ):
...
def hard_reset():
    ...
    hard_reset_sysrq_cmd = get_adb_command_line("shell echo b \\> /proc/sysrq-trigger")
...
def start_gce_device():
    ...
    device_memory_mb = environment.get_value(
        "DEVICE_MEMORY_MB", DEFAULT_DEVICE_MEMORY_MB
    )
    launch_cvd_command_line = "{launch_cvd_path} -daemon -memory_mb {device_memory_mb}".format(
...
def reset_device_connection():
    ...
    if state != "device":
        logs.log_warn(
            "Device state is %s, unable to recover using usb reset/"
...
def get_device_path():
    ...
    def _get_usb_devices():
        ...
        if output is None:
            logs.log_error(
                "Failed to populate usb devices using lsusb, "
...
def run_command(cmd, log_output=False, log_error=True, timeout=None, recover=True):
...
def run_shell_command(
    cmd, log_output=False, log_error=True, root=False, timeout=None, recover=True
):
...
def write_command_line_file(command_line, app_path):
    ...
    command_line_file_contents = "chrome %s" % (command_line_without_app_path.strip())
