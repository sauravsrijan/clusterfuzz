...
SANITIZER_TOOL_TO_FILE_MAPPINGS = {"asan": "asan.options"}
...
def get_options_file_path(sanitizer_tool_name):
    ...
    sanitizer_directory = (
        "/system" if settings.get_sanitizer_tool_name() else constants.DEVICE_TMP_DIR
    )
...
def setup_asan_if_needed():
    ...
    asan_device_setup_script_path = os.path.join(
        android_directory, "third_party", "asan_device_setup.sh"
    )
    ...
    process = new_process.ProcessRunner(
        asan_device_setup_script_path, asan_device_setup_script_args
    )
