...
def get_stacktrace(testcase, stack_attribute="crash_stacktrace"):
  ...
  try:
    ...
    logs.log_error("Unable to read stacktrace for testcase %d." %
                   testcase.key.id())
    result = ""
...
def _get_security_severity(crash, job_type, gestures):
  ...
  if crash.security_flag:
    return severity_analyzer.get_security_severity(crash.crash_type,
                                                   crash.crash_stacktrace,
                                                   job_type, bool(gestures))
...
def update_testcase_comment(testcase, task_state, message=None):
  ...
  if len(testcase.comments) > data_types.TESTCASE_COMMENTS_LENGTH_LIMIT:
    ...
    testcase.comments = testcase.comments[-data_types
                                          .TESTCASE_COMMENTS_LENGTH_LIMIT:]
