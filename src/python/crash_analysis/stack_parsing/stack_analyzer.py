...
LINUX_GDB_CRASH_TYPE_REGEX = re.compile(
...
RUNTIME_ERROR_LINE_REGEX = re.compile(
...
UBSAN_INVALID_BUILTIN_REGEX = re.compile(
...
def filter_crash_parameters(state):
    ...
    if state.crash_address and state.crash_type.startswith('UNKNOWN'):
        ...
        if crash_analyzer.is_null_dereference(int_crash_address):
            state.crash_type = state.crash_type.replace(
                'UNKNOWN', 'Null-dereference')
...
def fix_win_cdb_crash_type(crash_type):
    ...
    crash_type = crash_type.replace(
        'Status-integer-overflow', 'Integer-overflow')
...
def fix_check_failure_string(failure_string):
    ...
    failure_string = utils.strip_from_left(
        failure_string, CHECK_FAILURE_PATTERN)
    ...
    for frame_index in range(num_frames):
        for cycle_length in range(1, MAX_CYCLE_LENGTH + 1):
            ...
            for i in range(0, REPEATED_CYCLE_COUNT):
                ...
                frame_potential_cycles.append(
                    state.raw_frames[start_index:end_index])
...
def get_crash_data(crash_data, symbolize_flag=True):
    ...
    if is_python:
        split_crash_stacktrace = reverse_python_stacktrace(
            split_crash_stacktrace)
    ...
    for line in split_crash_stacktrace:
        ...
        if (state.crash_type == 'Bad-cast' and
            ...
            if downcast_match:
                state.crash_state += ' from %s' % (
                    downcast_match.group(1)).strip("'")
        ...
        if state_needs_change:
            ...
            if android_segv_match:
                ...
                if process_name_match:
                    state.process_name = process_name_match.group(
                        1).capitalize()
        ...
        if not state.crash_type:
            ...
            if fatal_error_match:
                ...
                state.crash_state = filter_stack_frame(
                    fatal_error_match.group(1))
            ...
            if is_golang:
                ...
                if golang_fatal_error_match:
                    state.crash_state = golang_fatal_error_match.group(
                        1) + '\n'
