...
ASAN_MEMCPY_OVERLAP_REGEX = re.compile(
    r".*(AddressSanitizer).*memcpy-param-overlap" r"[^\[]*([\[].*[)])"
)
...
ASSERT_REGEX = re.compile(
    ...
    + r"\s*(?:ASSERT(?:ION)? FAIL(?:URE|ED)|panic): (.*)",
    re.IGNORECASE,
)
ASSERT_REGEX_GOOGLE = re.compile(
...
MSAN_TSAN_REGEX = re.compile(r".*(ThreadSanitizer|MemorySanitizer):[ ]*([^(:]+)")
...
GENERIC_SEGV_HANDLER_REGEX = re.compile("Received signal 11 SEGV_[A-Z]+ ([0-9a-f]*)")
GOOGLE_CHECK_FAILURE_REGEX = re.compile(
...
LIBRARY_NOT_FOUND_ANDROID_REGEX = re.compile(r'.*: library ([`\'"])(.*)\1 not found')
...
MAC_GDB_CRASH_ADDRESS_REGEX = re.compile(r"Reason:.*at address[^0-9]*([0-9a-zA-Z]+)")
...
SAN_CHECK_FAILURE_REGEX = re.compile(r".*Sanitizer CHECK failed[:]\s*[^ ]*\s*(.*)")
...
UBSAN_FLOAT_CAST_OVERFLOW_REGEX = re.compile(
    r".*outside the range of " r"representable values.*"
)
...
UBSAN_UNSIGNED_INTEGER_OVERFLOW_REGEX = re.compile(r".*unsigned integer overflow.*")
UBSAN_INTEGER_OVERFLOW_REGEX = re.compile(
    r".*(integer overflow|" r"(negation|division) of.*cannot be represented in type).*"
)
...
UBSAN_VPTR_INVALID_DOWNCAST_REGEX = re.compile(r".*note: object is of type (.*)")
...
WINDOWS_CDB_CRASH_TYPE_ADDRESS_REGEX = re.compile(r"Attempt to (.*) [^ ]* address (.*)")
WINDOWS_CDB_CRASH_TYPE_REGEX = re.compile(r".*DEFAULT_BUCKET_ID[ ]*[:][ ]*([a-zA-Z_]+)")
...
CHROME_STACK_FRAME_SPEC = stack_parser.StackFrameSpec(address=3, function_name=4)
CHROME_WIN_STACK_FRAME_SPEC = stack_parser.StackFrameSpec(
    function_name=1, function_base=2, function_offset=3, filename=5, fileline=6, base=10
)
...
class StackAnalyzerState(object):
    ...
    def __init__(self, symbolized=True):
        ...
        self.stack_frame_ignore_regex = re.compile(
            r"(%s)"
            % "|".join(STACK_FRAME_IGNORE_REGEXES + custom_stack_frame_ignore_regexes)
        )
...
def filter_crash_parameters(state):
    ...
    state.crash_state = "".join(s for s in state.crash_state if s in string.printable)
    ...
    if (
        state.crash_state
        and state.crash_state != "NULL"
        and state.crash_state[-1] != "\n"
    ):
        state.crash_state += "\n"
    ...
    if m:
        ...
        state.crash_type = (
            state.crash_type[: len(m.group(1))] + str(num) + state.crash_type[m.end() :]
        )

    # On some platforms crash address is unnecessarily long. We can truncate it.
    if state.crash_address.startswith("0x0000") and len(state.crash_address) == 18:
...
def update_state_on_match(
    ...
    if address_from_group is not None:
        state.crash_address = address_filter(match.group(address_from_group)).strip()
...
def add_frame_on_match(
    ...
    if frame_spec is not None:
        ...
        if (
            frame_struct
            and frame_struct.module_offset
            and not frame_struct.function_name
        ):
...
def fix_win_cdb_crash_type(crash_type):
    ...
    crash_type = crash_type.replace("Status-integer-divide-by-zero", "Divide-by-zero")
    return crash_type
        ...
        if in_python_stacktrace is False:
            for regex, _ in PYTHON_CRASH_TYPES_MAP:
                if regex.match(line):
                    ...
                    # Add the "title" of the stacktrace
                    python_stacktrace_split = [line]
                    break
        else:
...
def get_crash_data(crash_data, symbolize_flag=True):
    ...
    detect_ooms_and_hangs = environment.get_value("REPORT_OOMS_AND_HANGS") and (
    ...
    ubsan_disabled = "halt_on_error=0" in environment.get_value("UBSAN_OPTIONS", "")
    ...
    for line in split_crash_stacktrace:
        ...
        if (
            SAN_ABRT_REGEX.match(line)
            or SAN_BREAKPOINT_REGEX.match(line)
            or SAN_ILL_REGEX.match(line)
        ):
        ...
        if not state.crash_type or state.crash_type == "Indirect-leak":
            update_state_on_match(
                LSAN_DIRECT_LEAK_REGEX, line, state, new_type="Direct-leak", reset=True
            )
        ...
        if not state.crash_type and not ubsan_disabled:
            ...
            if ubsan_vptr_match:
                state.crash_state = "Bad-cast to %s" % (
                    ubsan_vptr_match.group(4)
                ).strip("'")
                state.found_bad_cast_crash_end_marker = False

        # Get source type information for bad-cast.
        if state.crash_type == "Bad-cast" and not state.found_bad_cast_crash_end_marker:
            ...
            if UBSAN_VPTR_INVALID_VPTR_REGEX.match(
                line
            ) or CFI_INVALID_VPTR_REGEX.match(line):
            ...
            if invalid_offset_match:
                state.crash_state += (
                    " from base class subobject at offset %s"
        ...
        if segv_match:
            ...
            if "ASSERT" in state.crash_type:
                ...
                if crash_analyzer.is_assert_crash_address(
                    int_crash_address
                ) or SAN_SIGNAL_REGEX.match(crash_data):
                    continue
        ...
        if update_state_on_match(
            SAN_FPE_REGEX, line, state, new_type="Floating-point-exception", reset=True
        ):
            continue
        ...
        if crash_type_and_address_match and not state.crash_type.startswith("UNKNOWN"):
            state.crash_type += "\n%s %s" % (
                crash_type_and_address_match.group(1).upper(),
                crash_type_and_address_match.group(2),
            )
        ...
        state_needs_change = (
            not state.crash_type.startswith("UNKNOWN") or "Fatal signal" not in line
        )
        ...
        android_abort_match = update_state_on_match(
            ANDROID_ABORT_REGEX, line, state, new_type="CHECK failure", new_address=""
        )
        ...
        if state.crash_type.startswith("Kernel failure"):
            ...
            if kasan_access_match:
                state.crash_type += " %s %s" % (
                    kasan_access_match.group(1).upper(),
        ...
        if detect_ooms_and_hangs:
            ...
            update_state_on_match(
                OUT_OF_MEMORY_REGEX, line, state, new_type="Out-of-memory", reset=True
            )
        ...
        if not state.crash_type:
            ...
            update_state_on_match(
                WINDOWS_CDB_STACK_OVERFLOW_REGEX, line, state, new_type="Stack-overflow"
            )
            ...
            # Check failures.
            update_state_on_check_failure(
                state, line, GOOGLE_LOG_FATAL_REGEX, "Fatal error"
            )
            update_state_on_check_failure(
            ...
            if detect_v8_runtime_errors:
                runtime_error_match = update_state_on_match(
                    RUNTIME_ERROR_REGEX,
                    line,
                    state,
        ...
        update_state_on_match(
            WINDOWS_CDB_STACK_START_REGEX, line, state, new_state="", new_frame_count=0
        )

        # Stack frame parsing signatures.
        # Don't allow more stack frames if a certain stop marker is seen.
        if state.crash_state and utils.sub_string_exists_in(STATE_STOP_MARKERS, line):
            state.frame_count = MAX_CRASH_STATE_FRAMES
            continue
        ...
        if (
            state.found_java_exception
            and state.crash_type in ["CHECK failure", "Fatal Exception"]
            and add_frame_on_match(
