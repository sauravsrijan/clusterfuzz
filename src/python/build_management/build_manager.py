...
BUILD_TYPE_SUBSTRINGS = ["-beta", "-stable", "-debug", "-release", "-symbolized"]
...
FUZZ_TARGET_EXCLUDED_EXTENSIONS = ["exe", "options", "txt", "zip", "exe.pdb", "par"]
...
def _make_space_for_build(
    build_local_archive, current_build_dir, file_match_callback=None
):
...
def _evict_build(current_build_dir):
    ...
    for build_directory in os.listdir(builds_directory):
        ...
        if (
            least_recently_used_timestamp is None
            or timestamp < least_recently_used_timestamp
        ):
    ...
    logs.log("Deleting build %s to save space." % least_recently_used.base_build_dir)
...
def _get_file_match_callback():
    ...
    def file_match_callback(filepath):
        ...
        if any(
            component.startswith(whitelisted_names) for component in path_components
        ):
            return True
...
def _setup_build_directories(base_build_dir):
    ...
    build_directories = [
        custom_binary_directory,
        revision_build_directory,
        sym_build_directory,
        sym_debug_build_directory,
        sym_release_build_directory,
    ]
    for build_directory in build_directories:
        shell.create_directory(build_directory)


def set_environment_vars(search_directories, app_path="APP_PATH", env_prefix=""):
    ...
    for search_directory in search_directories:
        for root, _, files in shell.walk(search_directory):
            ...
            for filename in files:
                ...
                if (
                    not llvm_symbolizer_path
                    and filename == llvm_symbolizer_filename
                    and not environment.get_value("USE_DEFAULT_LLVM_SYMBOLIZER")
                ):
                    llvm_symbolizer_path = os.path.join(root, llvm_symbolizer_filename)
                    environment.set_value(
                        env_prefix + "LLVM_SYMBOLIZER_PATH", llvm_symbolizer_path
                    )
...
class Build(BaseBuild):
    ...
    def _unpack_build(self, base_build_dir, build_dir, build_url, target_weights=None):
        ...
        if not unpack_everything:
            ...
            _set_random_fuzz_target_for_fuzzing_if_needed(
                self._get_fuzz_targets_from_archive(build_local_archive), target_weights
            )
        ...
        if not _make_space_for_build(
            build_local_archive, base_build_dir, file_match_callback
        ):
            shell.clear_data_directories()
            logs.log_fatal_and_exit(
        ...
        log_func("Build took %0.02f minutes to unpack." % (elapsed_time / 60.0))
    ...
    def _get_fuzz_targets_from_archive(self, archive_path):
        ...
        for archive_file in archive.iterator(archive_path):
            if fuzzer_utils.is_fuzz_target_local(
                archive_file.name, archive_file.handle
            ):
    ...
    def _setup_application_path(
        self, build_dir=None, app_path="APP_PATH", build_update=False
    ):
        ...
        environment.set_value(
            self.env_prefix + "LLVM_SYMBOLIZER_PATH",
        ...
        symbolic_link_target = environment.get_value(self.env_prefix + "SYMBOLIC_LINK")
...
class RegularBuild(Build):
    ...
    def __init__(
        self, base_build_dir, revision, build_url, target_weights=None, build_prefix=""
    ):
    ...
    def setup(self):
        ...
        if build_update:
            if not self._unpack_build(
                self.base_build_dir, self.build_dir, self.build_url, self.target_weights
            ):
                return False
...
class FuchsiaBuild(RegularBuild):
    ...
    SYMBOLIZE_REL_PATH = os.path.join(
        "build", "zircon", "prebuilt", "downloads", "symbolize"
    )
    LLVM_SYMBOLIZER_REL_PATH = os.path.join(
    ...
    def setup(self):
        ...
        llvm_symbolizer_path = os.path.join(
            self.build_dir, self.LLVM_SYMBOLIZER_REL_PATH
        )
...
class SymbolizedBuild(Build):
    ...
    def __init__(self, base_build_dir, revision, release_build_url, debug_build_url):
    ...
    def _unpack_builds(self):
        ...
        if self.release_build_url:
            if not self._unpack_build(
                self.base_build_dir, self.release_build_dir, self.release_build_url
            ):
                return False

        if self.debug_build_url:
            if not self._unpack_build(
                self.base_build_dir, self.debug_build_dir, self.debug_build_url
            ):
                return False
...
class ProductionBuild(Build):
    ...
    def setup(self):
        ...
        if build_update:
            if not self._unpack_build(
                self.base_build_dir, self.build_dir, self.build_url
            ):
                return False
...
class CustomBuild(Build):
    ...
    def _unpack_custom_build(self):
        ...
        build_local_archive = os.path.join(self.build_dir, self.custom_binary_filename)
...
def _sort_build_urls_by_revision(build_urls, bucket_path, reverse):
    ...
    for build_url in build_urls:
        match_pattern = "{base_path_with_seperator}({file_pattern})".format(
            base_path_with_seperator=base_path_with_seperator, file_pattern=file_pattern
        )
        ...
        if match:
            ...
            if revision in filename_by_revision_dict:
                ...
                raise errors.BadStateError(
                    ...
                    "Bucket path is probably malformed for job %s."
                    % (revision, job_name)
                )
...
def get_build_urls_list(bucket_path, reverse=True):
    ...
    if environment.is_running_on_app_engine():
        ...
        content = utils.read_data_from_file(keys_file_path, eval_data=False).decode(
            "utf-8"
        )
        if not content:
            return []
...
def setup_regular_build(
    revision, bucket_path=None, build_prefix="", target_weights=None
):
    """Sets up build with a particular revision."""
    if not bucket_path:
...
def setup_symbolized_builds(revision):
    ...
    sym_debug_build_bucket_path = environment.get_value("SYM_DEBUG_BUILD_BUCKET_PATH")
    ...
    sym_debug_build_url = revisions.find_build_url(
        sym_debug_build_bucket_path, sym_debug_build_urls, revision
    )
    ...
    build = build_class(
        base_build_dir, revision, sym_release_build_url, sym_debug_build_url
    )
    if build.setup():
        return build
...
def setup_production_build(build_type):
    ...
    if not build_urls:
        logs.log_error("Error getting list of build urls from %s." % build_bucket_path)
        return None
...
def is_custom_binary():
    ...
    return environment.get_value("CUSTOM_BINARY") or environment.get_value(
        "SYSTEM_BINARY_DIR"
    )
...
def has_production_builds():
    ...
    return environment.get_value("STABLE_BUILD_BUCKET_PATH") and environment.get_value(
...
def has_symbolized_builds():
    ...
    return environment.get_value(
        "SYM_RELEASE_BUILD_BUCKET_PATH"
    ) or environment.get_value("SYM_DEBUG_BUILD_BUCKET_PATH")
...
def get_rpaths(binary_path):
    ...
    try:
        rpaths = (
            subprocess.check_output(
                [chrpath, "-l", binary_path], stderr=subprocess.PIPE
            )
            .strip()
            .decode("utf-8")
        )
...
def check_app_path(app_path="APP_PATH"):
    ...
    # APP_PATH.
    return not environment.get_value("APP_NAME") or environment.get_value(app_path)
