...
def _evict_build(current_build_dir):
  ...
  logs.log("Deleting build %s to save space." %
           least_recently_used.base_build_dir)
...
def _setup_split_targets_build(bucket_path, target_weights, revision=None):
  ...
  if not targets_list:
    raise BuildManagerException("No targets found in targets.list (path=%s)." %
                                bucket_path)

  fuzz_target = _set_random_fuzz_target_for_fuzzing_if_needed(
      targets_list, target_weights)
  if not fuzz_target:
    raise BuildManagerException("Failed to choose a fuzz target (path=%s)." %
                                bucket_path)
...
def setup_regular_build(revision,
  ...
  if not build_url:
    logs.log_error("Error getting build url for job %s (r%d)." %
                   (job_type, revision))
...
def setup_production_build(build_type):
  ...
  if not build_urls:
    logs.log_error("Error getting list of build urls from %s." %
                   build_bucket_path)
    return None
  ...
  if not v_match:
    logs.log_error("Unable to find version information from the build url %s." %
                   build_url)
    return None
...
def has_production_builds():
  ...
  return environment.get_value(
      "STABLE_BUILD_BUCKET_PATH") and environment.get_value(
...
def has_symbolized_builds():
  """Return a bool on if job type has either a release or debug build for stack
    symbolization."""
  return environment.get_value(
      "SYM_RELEASE_BUILD_BUCKET_PATH") or environment.get_value(
          "SYM_DEBUG_BUILD_BUCKET_PATH")
...
def _set_rpaths_chrpath(binary_path, rpaths):
  ...
  subprocess.check_output([chrpath, "-r", ":".join(rpaths), binary_path],
...
def get_rpaths(binary_path):
  ...
  try:
    rpaths = subprocess.check_output([chrpath, "-l", binary_path],
...
def check_app_path(app_path="APP_PATH"):
  ...
  # APP_PATH.
  return not environment.get_value("APP_NAME") or environment.get_value(
      app_path)
