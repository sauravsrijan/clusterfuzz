...
def _evict_build(current_build_dir):
    ...
    for build_directory in os.listdir(builds_directory):
        absolute_build_directory = os.path.join(
            builds_directory, build_directory)
...
def _get_build_directory(bucket_path, job_name):
    ...
    if bucket_path:
        ...
        file_pattern = utils.remove_sub_strings(
            file_pattern, BUILD_TYPE_SUBSTRINGS)
...
def _set_random_fuzz_target_for_fuzzing_if_needed(fuzz_targets, target_weights):
    ...
    fuzz_target = environment.get_value('FUZZ_TARGET')
    if fuzz_target:
        logs.log('Use previously picked fuzz target %s for fuzzing.' %
                 fuzz_target)
        return fuzz_target
            ...
            for filename in files:
                if not absolute_file_path and filename == app_name:
                    ...
                    environment.set_value(
                        env_prefix + app_path, absolute_file_path)
                    environment.set_value(
                        env_prefix + 'APP_DIR', app_directory)

                if not gn_args_path and filename == gn_args_filename:
                    gn_args_path = os.path.join(root, gn_args_filename)
                    environment.set_value(
                        env_prefix + 'GN_ARGS_PATH', gn_args_path)
...
class BaseBuild(object):
    ...
    def last_used_time(self):
        ...
        timestamp = utils.read_data_from_file(
            timestamp_file_path, eval_data=True)
...
class Build(BaseBuild):
    ...
    def _delete_partial_build_file(self):
        ...
        partial_build_file_path = os.path.join(
            self.build_dir, PARTIAL_BUILD_FILE)
    ...
    def _post_setup_success(self, update_revision=True):
        ...
        if self.base_build_dir:
            timestamp_file_path = os.path.join(
                self.base_build_dir, TIMESTAMP_FILE)
    ...
    def _unpack_build(self,
        ...
        build_local_archive = os.path.join(
            build_dir, os.path.basename(build_url))
        ...
        try:
            ...
            logs.log_error('Unable to unpack build archive %s.' %
                           build_local_archive)
            return False
    ...
    def _get_fuzz_targets_from_archive(self, archive_path):
        ...
        for archive_file in archive.iterator(archive_path):
            if fuzzer_utils.is_fuzz_target_local(archive_file.name,
                                                 archive_file.handle):
                fuzz_target = os.path.splitext(
                    os.path.basename(archive_file.name))[0]
                yield fuzz_target
    ...
    def _setup_application_path(self,
        ...
        if symbolic_link_target:
            os.system('mkdir --parents %s' %
                      os.path.dirname(symbolic_link_target))
        ...
        android.device.update_build(
            absolute_file_path, force_update=build_update)
...
class RegularBuild(Build):
    ...
    def __init__(self,
                 ...
                 target_weights=None,
                 build_prefix=''):
        super(RegularBuild, self).__init__(
            base_build_dir, revision, build_prefix)
        ...
        self._build_dir = os.path.join(
            self.base_build_dir, self.build_dir_name)
...
class FuchsiaBuild(RegularBuild):
    ...
    def _get_fuzz_targets_from_dir(self, build_dir):
        ...
        host = Host.from_dir(os.path.join(
            build_dir, self.FUCHSIA_BUILD_REL_PATH))
...
class ProductionBuild(Build):
    ...
    def setup(self):
        """Sets up build with a particular revision."""
        self._pre_setup()
        logs.log('Retrieving %s branch (%s).' %
                 (self.build_type, self.revision))
        ...
        if build_update:
            ...
            revisions.write_revision_to_revision_file(
                version_file, self.revision)
            logs.log('Retrieved %s branch (%s).' %
                     (self.build_type, self.revision))
        else:
            logs.log('Build already exists.')
...
class CustomBuild(Build):
    ...
    def __init__(self,
                 ...
                 target_weights=None):
        super(CustomBuild, self).__init__(
            base_build_dir, custom_binary_revision)
    ...
    def _unpack_custom_build(self):
        ...
        if archive.is_archive(self.custom_binary_filename):
            ...
            try:
                archive.unpack(build_local_archive,
                               self.build_dir, trusted=True)
            except:
                logs.log_error(
...
def _get_latest_revision(bucket_paths):
    ...
    for bucket_path in bucket_paths:
        ...
        if not urls_list:
            logs.log_error(
                'Error getting list of build urls from %s.' % bucket_path)
            return None

        build_urls.append(
            BuildUrls(bucket_path=bucket_path, urls_list=urls_list))
