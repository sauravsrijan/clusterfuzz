...
def merge_steps(no_track_origins=False):
    ...
    return [
        ...
        {
            # Zip.
            "args": ["bash", "-c", "find -L -name '*.so*' | zip -y %s -@" % filename],
            "dir": "all",
            "name": "gcr.io/oss-fuzz-base/base-msan-builder",
        },
        {
            # Upload.
            "args": ["cp", filename, bucket_path(no_track_origins) + "/" + filename,],
            "dir": "all",
            "name": "gcr.io/cloud-builders/gsutil",
        },
    ]
...
def start_build(cloudbuild, build_body):
    ...
    build_info = (
        cloudbuild.projects()
        .builds()
        .create(projectId="google.com:clusterfuzz", body=build_body)
        .execute()
    )
...
def main():
    parser = argparse.ArgumentParser("build_msan_libs.py", description="MSan builder.")
    parser.add_argument(
        "--no-track-origins",
        action="store_true",
    ...
    parser.add_argument(
        "command", choices=["build_packages", "merge"], help="The command to run."
    )
