...
def main():
    ...
    builds_metadata = build_info.get_production_builds_info(environment.platform())
    ...
    for build_metadata in builds_metadata:
        ...
        for tool in TOOLS_GN_MAPPINGS:
            ...
            if (
                tool_and_build_type in LAST_BUILD
                and revision == LAST_BUILD[tool_and_build_type]
            ):
                logs.log(
                    "Skipping same build %s (revision %s)."
            ...
            archive_path_remote = "gs://%s/%s/%s" % (
                bucket_name,
                TOOLS_BUCKET_DIR_MAPPINGS[tool],
                archive_filename,
            )
