...
class CrashReportInfo(object):
    ...
    def upload(self):
        ...
        report_file = FileMetadataInfo(
            contents=self.serialized_crash_stack_frames
        ).get_file_handle()
        ...
        if report_file is None and minidump_file is None:
            logs.log_error(
                "Neither processed report nor minidump, nothing to " "upload."
            )
            return None
...
def crash_report_info_from_metadata(report_metadata):
    ...
    return CrashReportInfo(
        ...
        serialized_crash_stack_frames=(report_metadata.serialized_crash_stack_frames),
...
def parse_mime_to_crash_report_info(local_minidump_mime_path):
    ...
    if minidump_path_match is None:
        logs.log_error(
            "Minidump filename in unexpected format: '%s'." % local_minidump_mime_path
        )
        return None
    ...
    for mime_part in minidump_mime_contents.get_payload():
        if isinstance(mime_part, str):
            ...
            logs.log_error(
                "Unexpected str mime_part from mime path %s: %s"
        ...
        if platform == "ANDROID":
            ...
            for device_directory in device_directories_to_search:
                device_minidump_mime_potential_paths = adb.run_shell_command(
                    ["ls", '"%s"' % device_directory], root=True
                ).splitlines()
            ...
            if device_minidump_mime_path is None:
                logs.log_error(
                    "Could not get MIME path from ls:\n%s"
            ...
            local_minidump_mime_path = os.path.join(
                crash_stacks_directory, minidump_mime_filename
            )
            adb.run_command(
                ["pull", '"%s"' % device_minidump_mime_path, local_minidump_mime_path]
            )
...
def get_crash_info_and_stacktrace(application_command_line, crash_stacktrace, gestures):
    ...
    if not crash_info or not crash_info.minidump_info.path:
        for _ in range(retry_limit):
            _, _, output = process_handler.run_process(
                application_command_line, timeout=warmup_timeout, gestures=gestures
            )
...
def get_symbolized_stack_bytes(crash_type, crash_address, symbolized_stack):
    ...
    process_state = process_state_pb2.ProcessStateProto(
        crash=process_state_pb2.ProcessStateProto.Crash(
            reason=crash_type, address=stack_parser.unsigned_to_signed(crash_address)
        ),
        requesting_thread=0,
    )
...
def save_crash_info_if_needed(
    testcase_id, crash_revision, job_type, crash_type, crash_address, crash_frames
):
