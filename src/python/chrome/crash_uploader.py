...
def post_with_retries(upload_url, params, files):
  ...
  for _ in range(retry_limit):
    try:
      ...
      logs.log_error("Failed to upload request, error code %d." %
                     result.status_code)
      return None
    except Exception:
...
class CrashReportInfo(object):
  ...
  def upload(self):
    ...
    if not mode or mode not in CRASH_REPORT_UPLOAD_URL:
      logs.log_warn("Missing or unknown mode (%s); uploading to staging." %
                    str(mode))
      mode = "staging"
...
def parse_mime_to_crash_report_info(local_minidump_mime_path):
  ...
  if PRODUCT_KEY in mime_key_values:
    ...
    logs.log_error("Could not find '%s' or alias in mime_key_values key." %
                   PRODUCT_KEY)
