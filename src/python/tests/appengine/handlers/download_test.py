...
class DownloadTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app = webtest.TestApp(
            webapp2.WSGIApplication(
                [("/download/?([^/]+)?", download.Handler),], debug=True
            )
        )
    ...
    def test_download_not_logged_in(self):
        ...
        self._test_download(self.minimized_key, expect_status=302, expect_blob=False)
        ...
        self._test_download(
            testcase_id=self.testcase.key.id(), expect_status=302, expect_blob=False
        )
        self._test_download(
            self.minimized_key,
    ...
    def test_download_testcase_user(self):
        ...
        self._test_download(self.minimized_key, expect_status=403, expect_blob=False)
    ...
    def test_download_testcase_no_user_access(self):
        ...
        self.mock.can_user_access_testcase.return_value = False
        self._test_download(
            testcase_id=self.testcase.key.id(), expect_status=403, expect_blob=False
        )
        self._test_download(
            self.minimized_key,
            testcase_id=self.testcase.key.id(),
    ...
    def test_download_invalid_id(self):
        ...
        self._test_download(
            self.minimized_key, testcase_id=1337, expect_blob=False, expect_status=400
        )
