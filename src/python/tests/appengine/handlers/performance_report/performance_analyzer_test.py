...
def _get_stats_from_log(
    log_path, strategies=None, arguments=None, stats_overrides=None
):
    ...
    log_lines = utils.decode_to_unicode(
        utils.read_data_from_file(log_path, eval_data=False)
    ).splitlines()
    stats = libfuzzer.parse_log_stats(log_lines)
    stats.update(
        performance_stats.parse_performance_features(log_lines, strategies, arguments)
    )
    if stats_overrides:
...
class PerformanceAnalyzerTestBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.libfuzzer_data_directory = os.path.join(self.data_directory, "libfuzzer")
...
class PerformanceAnalyzerBasicAnalyzerTest(PerformanceAnalyzerTestBase):
    ...
    def assert_basic_analyzer(self, basic_analyzer, log_filename, stats_overrides=None):
        ...
        log_file_path = os.path.join(
            self.libfuzzer_data_directory, "issue_logs", log_filename
        )
    ...
    def test_basic_analyzer_for_bad_instrumentation(self):
        ...
        self.assert_basic_analyzer(
            self.analyzer.analyzer_bad_instrumentation, "bad_instrumentation_issue.txt"
        )

    def test_basic_analyzer_for_coverage(self):
        """Test analyzer_coverage BasicAnalyzer."""
        self.assert_basic_analyzer(
            self.analyzer.analyzer_coverage, "coverage_issue.txt"
        )
    ...
    def test_basic_analyzer_for_logging(self):
        ...
        self.assert_basic_analyzer(self.analyzer.analyzer_logging, "logging_issue.txt")
    ...
    def test_basic_analyzer_for_slow_unit(self):
        ...
        self.assert_basic_analyzer(
            self.analyzer.analyzer_slow_unit, "slow_unit_issue.txt"
        )
    ...
    def test_basic_analyzer_for_startup_crash(self):
        ...
        self.assert_basic_analyzer(
            self.analyzer.analyzer_startup_crash, "startup_crash_issue.txt"
        )

    def test_basic_analyzer_for_timeout(self):
        """Test analyzer_timeout BasicAnalyzer."""
        self.assert_basic_analyzer(self.analyzer.analyzer_timeout, "timeout_issue.txt")
...
class PerformanceAnalyzerTest(PerformanceAnalyzerTestBase):
    ...
    def get_issues(self, log_filename, stats_overrides=None):
        ...
        performance_scores, affected_runs_percents, examples = analyzer.analyze_stats(
            [stats]
        )

        return analyzer.get_issues(performance_scores, affected_runs_percents, examples)

    def assert_log_has_issue_matching(
        self, log_filename, expected_issue, stats_overrides=None
    ):
        """Assert for testing log has a particular issue."""
        detected_issues = self.get_issues(log_filename, stats_overrides)
        actual_issue = next(
            (i for i in detected_issues if i["type"] == expected_issue["type"]), None
        )
    ...
    def test_bad_instrumentation(self):
        ...
        self.assert_log_has_issue_matching(
            "bad_instrumentation_issue.txt", expected_issue
        )
    ...
    def test_startup_crash(self):
        ...
        self.assert_log_has_issue_matching(
            "startup_crash_issue.txt", expected_issue, stats_overrides
        )
    ...
    def test_no_startup_crash(self):
        ...
        self.assert_log_has_no_issue_matching(
            "startup_crash_no_issue.txt", "startup_crash"
        )
    ...
    def test_no_logging_with_crash(self):
        ...
        self.assert_log_has_no_issue_matching("logging_crash_no_issue.txt", "logging")

    def test_no_logging_with_slow_units(self):
        """Test no logging issue for a log with slow units."""
        self.assert_log_has_no_issue_matching(
            "logging_slow_units_no_issue.txt", "logging"
        )
    ...
    def test_no_logging_with_few_runs(self):
        ...
        self.assert_log_has_no_issue_matching(
            "logging_few_runs_no_issue.txt", "logging"
        )
    ...
    def test_corpus_subset_run_crash(self):
        ...
        expected_issue = {"type": "crash", "percent": 100.0, "score": 4.0}
        self.assert_log_has_issue_matching(
            "corpus_subset_crash_issue.txt", expected_issue
        )
    ...
    def test_report_generation(self):
        ...
        analyzer = performance_analyzer.LibFuzzerPerformanceAnalyzer()
        report_logs_directory = os.path.join(
            self.libfuzzer_data_directory, "report_logs"
        )
        stats_rows = []
        ...
        self.assertEqual(json.loads(performance_report), json.loads(expected_report))
