...
class OssFuzzGenerateCertsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_helpers.patch(self, ["handlers.base_handler.Handler.is_cron",])

        self.mock.is_cron.return_value = True
        self.app = webtest.TestApp(
            webapp2.WSGIApplication(
                [("/generate-certs", oss_fuzz_generate_certs.Handler)]
            )
        )
    ...
    def test_execute(self):
        ...
        private_key = crypto.load_privatekey(crypto.FILETYPE_PEM, tls_cert.key_contents)
