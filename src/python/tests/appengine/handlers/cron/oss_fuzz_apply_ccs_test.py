...
OSS_FUZZ_POLICY = issue_tracker_policy.IssueTrackerPolicy(
    {
        "deadline_policy_message": "This bug is subject to a 90 day disclosure deadline. "
        "If 90 days elapse\n"
...
class OssFuzzApplyCcsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data_types.Testcase(
            open=True, status="Processed", bug_information="1337", job_type="job"
        ).put()

        data_types.Testcase(
            open=True, status="Processed", bug_information="1338", job_type="job"
        ).put()

        data_types.Testcase(
            open=True, status="Processed", bug_information="1339", job_type="job"
        ).put()

        data_types.Testcase(
            open=True, status="Processed", bug_information="1340", job_type="job"
        ).put()
    ...
    def test_execute(self):
        ...
        six.assertCountEqual(
            self, issue_1337.cc, ["user@example.com", "user2@example.com",]
        )
        ...
        six.assertCountEqual(
            self, issue_1339.cc, ["user@example.com", "user2@example.com",]
        )
        ...
        six.assertCountEqual(
            self, issue_1340.cc, ["user@example.com", "user2@example.com",]
        )
