...
INSTANCE_GROUPS = {
    "oss-fuzz-linux-zone2-pre-proj2": {"targetSize": 1,},
    "oss-fuzz-linux-zone2-pre-proj3": {"targetSize": 499,},
    "oss-fuzz-linux-zone2-pre-proj4": {"targetSize": 99,},
    "oss-fuzz-linux-zone2-pre-proj5": {"targetSize": 99,},
}
...
INSTANCE_TEMPLATES = {
    "oss-fuzz-linux-zone2-pre-proj2": {
        ...
        "properties": {
            "metadata": {"items": [],},
            "disks": [{"initializeParams": {"diskSizeGb": "30",},}],
            "serviceAccounts": [
                {
                    "email": "email",
                    "scopes": [
...
INSTANCES = {
    ...
    "oss-fuzz-linux-zone3-worker-high-end-proj6": [
        {
            "instance": (
                "https://blah/" "oss-fuzz-linux-zone3-worker-high-end-proj6-%04d" % i
            )
        }
        for i in range(1, 3)
    ],
...
class MockBotManager(object):
    ...
    def instance_group(self, name):
        ...
        resource = self._get_resource(
            name, self.instance_groups, INSTANCE_GROUPS, bot_manager.InstanceGroup
        )
...
def expected_instance_template(
    gce_project_name,
    name,
    project_name,
    disk_size_gb=None,
    ...
    expected["properties"]["metadata"]["items"].append(
        {"key": "task-tag", "value": project_name,}
    )
    ...
    if tls_cert:
        expected["properties"]["metadata"]["items"].extend(
            [
                {"key": "tls-cert", "value": project_name + "_cert",},
                {"key": "tls-key", "value": project_name + "_key",},
            ]
        )
...
class CronTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock.load_project.return_value = compute_engine_projects.Project(
            ...
            instance_templates=[
                ...
                },
                {
                    ...
                    "properties": {
                        "metadata": {"items": [],},
                        "disks": [{"initializeParams": {"diskSizeGb": 100,},}],
                        "serviceAccounts": [
                            {
                                "email": "email",
                                "scopes": [
        ...
        data_types.OssFuzzProject(
            ...
            name="proj3",
            cpu_weight=5.0,
            service_account="proj3@serviceaccount.com",
        ).put()
        ...
        data_types.OssFuzzProject(
            ...
            name="proj4",
            cpu_weight=1.0,
            service_account="proj4@serviceaccount.com",
        ).put()
        ...
        for j in range(1, 7):
            ...
            data_types.WorkerTlsCert(
                ...
                key_contents=project_name.encode() + b"_key",
            ).put()
        ...
        data_types.OssFuzzProjectInfo(
            ...
            clusters=[
                ...
                data_types.OssFuzzProjectInfo.ClusterInfo(
                    cluster="old-cluster", gce_zone="us-east2-a", cpu_count=1,
                ),
            ],
        ).put()
        ...
        data_types.HostWorkerAssignment(
            id="old-host-0", host_name="old-host", worker_name="worker", instance_num=0
        ).put()
    ...
    def test_update_cpus(self):
        ...
        mock_bot_manager = self.mock.BotManager("clusterfuzz-external", "us-east2-a")
        ...
        mock_bot_manager.instance_template(
            ...
            expected_instance_template(
                "clusterfuzz-external", "external-pre-zone2", "proj1"
            )
        )
        ...
        mock_bot_manager.instance_group(
            "oss-fuzz-linux-zone2-pre-proj2"
        ).resize.assert_called_with(200, wait_for_instances=False)
        mock_bot_manager.instance_template("old-cluster-proj2").delete.assert_called()
        ...
        mock_bot_manager.instance_template(
            ...
            expected_instance_template(
                "clusterfuzz-external", "external-pre-zone2", "proj4"
            )
        )
        mock_bot_manager.instance_group(
            "oss-fuzz-linux-zone2-pre-proj4"
        ...
        mock_bot_manager.instance_template(
            ...
            expected_instance_template(
                "clusterfuzz-external", "external-pre-zone2", "proj5", disk_size_gb=10
            )
        )
        mock_bot_manager.instance_group(
            "oss-fuzz-linux-zone2-pre-proj5"
        ...
        for j in range(1, 6):
            mock_bot_manager.instance_group(
                "oss-fuzz-linux-zone3-worker-high-end-proj" + str(j)
            ).create.assert_not_called()
        ...
        # old_proj: deleted.
        mock_bot_manager.instance_group(
            "oss-fuzz-linux-zone2-pre-old-proj"
        ).create.assert_not_called()
        mock_bot_manager.instance_template(
            "oss-fuzz-linux-zone2-pre-old-proj"
        ).delete.assert_called()
        ...
        mock_bot_manager.instance_template(
            ...
            expected_host_instance_template("clusterfuzz-external", "host-zone3")
        )
        mock_bot_manager.instance_group(
            "oss-fuzz-linux-zone3-host"
        ).create.assert_called_with(
...
class OssFuzzDistributeCpusTest(unittest.TestCase):
    ...
    def test_equal_uneven(self):
        ...
        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 31)
        self.assertListEqual([11, 10, 10], result)

        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 32)
    ...
    def test_weight_preference(self):
        ...
        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 4)
        self.assertListEqual([1, 1, 2], result)

        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 5)
    ...
    def test_not_enough(self):
        ...
        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 1)
        self.assertListEqual([1, 0, 0], result)

        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 2)
    ...
    def test_minimum(self):
        ...
        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 3)
        self.assertListEqual([1, 1, 1], result)

        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 10)
    ...
    def test_maximum(self):
        ...
        result = manage_vms.OssFuzzClustersManager(
            "clusterfuzz-external"
        ).distribute_cpus(projects, 10000)
