...
class TestFormatters(unittest.TestCase):
    ...
    def setUp(self):
        test_helpers.patch(self, ["base.utils.utcnow"])
...
class TestUpdateChildWeightsForParentFuzzer(unittest.TestCase):
    ...
    def test_reported_fuzzer_has_weight_restored(self):
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", binary="good_fuzzer", project="test-project"
        ).put()
        ...
        self.mock._query_helper.return_value = [
            {"fuzzer": "libFuzzer_good_fuzzer", "job": "asan", "new_weight": 1.00}
        ]
    ...
    def test_weight_increase(self):
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", binary="very_good_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
        ...
        # Report that the issue is corrected.
        self.mock._query_helper.return_value = [
            {"fuzzer": "libFuzzer_very_good_fuzzer", "job": "asan", "new_weight": 2.00}
        ]
    ...
    def test_problem_penalized(self):
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", binary="problematic_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
    ...
    def test_new_fuzzer(self):
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", binary="old_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", binary="new_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
