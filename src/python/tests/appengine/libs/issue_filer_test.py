...
CHROMIUM_POLICY = issue_tracker_policy.IssueTrackerPolicy(
    {
        ...
        "all": {"status": "new", "labels": ["ClusterFuzz", "Stability-%SANITIZER%"]},
        "non_security": {
            "labels": ["Type-Bug"],
        ...
        "security": {"labels": ["Type-Bug-Security"]},
        "existing": {"labels": ["Stability-%SANITIZER%"]},
    }
)
...
OSS_FUZZ_POLICY = issue_tracker_policy.IssueTrackerPolicy(
    {
        ...
        "all": {
            ...
            "issue_body_footer": "When you fix this bug, please\n"
        ...
        "security": {"labels": ["Type-Bug-Security"]},
        "deadline_policy_message": "This bug is subject to a 90 day disclosure deadline. If 90 days "
        "elapse\n"
...
class IssueFilerTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data_types.Job(
            name="chromeos_job4", environment_string="", platform="linux"
        ).put()
        ...
        self.testcase4 = data_types.Testcase(job_type="chromeos_job4", **testcase_args)
        self.testcase4.put()
    ...
    def test_filed_issues_chromium(self):
        ...
        self.assertNotIn(
            "Restrict-View-SecurityTeam", issue_tracker._itm.last_issue.labels
        )
    ...
    def test_filed_issues_oss_fuzz(self):
        ...
        self.assertTrue(
            issue_tracker._itm.last_issue.has_label_matching("restrict-view-commit")
        )
        self.assertFalse(
    ...
    @parameterized.parameterized.expand(
        [("chromium", CHROMIUM_POLICY), ("oss-fuzz", OSS_FUZZ_POLICY),]
    )
    ...
    def test_security_severity_security_bug_default_severity(
        ...
        self.testcase1.put()
        issue_filer.file_issue(self.testcase1, issue_tracker)
        self.assertIn("Security_Severity-High", issue_tracker._itm.last_issue.labels)
        self.assertEqual(
            1,
            len(
                issue_tracker._itm.last_issue.get_labels_by_prefix("Security_Severity-")
            ),
        )
    ...
    def test_security_severity_security_bug_severity_override(
        ...
        self.assertNotIn("Security_Severity-High", issue_tracker._itm.last_issue.labels)
        self.assertIn("Security_Severity-Medium", issue_tracker._itm.last_issue.labels)
        self.assertEqual(
            1,
            len(
                issue_tracker._itm.last_issue.get_labels_by_prefix("Security_Severity-")
    ...
    def test_security_severity_security_bug_with_severity_set(
        ...
        for security_severity in security_severity_string_map:
            ...
            issue_filer.file_issue(self.testcase1, issue_tracker)
            self.assertIn(
                security_severity_string_map[security_severity],
                issue_tracker._itm.last_issue.labels,
            )
            self.assertEqual(
                1,
    ...
    def test_memory_tool_used(self, project_name, policy):
        ...
        for entry in issue_filer.MEMORY_TOOLS_LABELS:
            ...
            issue_filer.file_issue(self.testcase1, issue_tracker)
            self.assertIn(
                "Stability-" + entry["label"], issue_tracker._itm.last_issue.labels
            )
...
class UpdateImpactTest(unittest.TestCase):
    ...
    def test_update_impact_stable_from_regression(self):
        ...
        six.assertCountEqual(self, ["Security_Impact-Stable"], mock_issue.labels.added)
    ...
    def test_update_impact_beta(self):
        ...
        issue_filer.update_issue_impact_labels(self.testcase, mock_issue)
        six.assertCountEqual(self, ["Security_Impact-Beta"], mock_issue.labels.added)
        six.assertCountEqual(self, [], mock_issue.labels.removed)
    ...
    def test_update_impact_head(self):
        ...
        issue_filer.update_issue_impact_labels(self.testcase, mock_issue)
        six.assertCountEqual(self, ["Security_Impact-Head"], mock_issue.labels.added)
        six.assertCountEqual(self, [], mock_issue.labels.removed)
    ...
    def test_replace_impact(self):
        ...
        issue_filer.update_issue_impact_labels(self.testcase, mock_issue)
        six.assertCountEqual(self, ["Security_Impact-Head"], mock_issue.labels.added)
        six.assertCountEqual(self, ["Security_Impact-Beta"], mock_issue.labels.removed)
