...
class FakeGSUtilRunner(object):
    ...
    def rsync(self, source, destination, exclusion_pattern=None):
        FakeGSUtilRunner.rsync_calls.append((source, destination, exclusion_pattern))
...
class UploadTestsToCloudStorageTest(fake_filesystem_unittest.TestCase):
    ...
    def test_tests_created_in_correct_bucket(self):
        ...
        files = [
            "/testcases/a/file1.txt",
            "/testcases/file2.txt",
            "/something/b/file3.txt",
            "/data/f/g/file4.txt",
        ]
        coverage_uploader.upload_testcases_if_needed(
            "test_fuzzer", files, "/testcases", "/data"
        )
    ...
    def test_large_testcase_list(self):
        ...
        filtered_files_list = "\n".join(["file%s" % i for i in range(1000)]).encode(
            "utf-8"
        )
        self.mock.write_data.assert_called_with(
            filtered_files_list,
