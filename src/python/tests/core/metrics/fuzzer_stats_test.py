...
class FuzzerStatsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        helpers.patch(self, ["google_cloud_utils.storage.write_data",])

    @test_utils.python2_only
    def test_upload_testcase_run(self):
        """Tests uploading of TestcaseRun."""
        testcase_run_0 = fuzzer_stats.TestcaseRun(
            "fuzzer", "job", 123, 1472846341.017923
        )
        testcase_run_1 = fuzzer_stats.TestcaseRun(
            "fuzzer", "job", 123, 1472846341.017923
        )
    ...
    def tests_upload_testcase_run_with_source(self):
        ...
        testcase_run = fuzzer_stats.TestcaseRun("fuzzer", "job", 123, 1472846341.017923)
    ...
    def test_upload_testcase_run_child(self):
        ...
        testcase_run_0 = fuzzer_stats.TestcaseRun(
            "parent_child", "job", 123, 1472846341.017923
        )
        testcase_run_0["stat"] = 1000
    ...
    def test_upload_testcase_run_2_days(self):
        ...
        testcase_run_1 = fuzzer_stats.TestcaseRun(
            "fuzzer", "job", 123, 1472846345.017923
        )
        testcase_run_2 = fuzzer_stats.TestcaseRun(
            "fuzzer", "job", 123, 1472932741.017923
        )
        testcase_run_3 = fuzzer_stats.TestcaseRun(
            "fuzzer", "job", 123, 1472932745.017923
        )
        testcase_run_0["stat"] = 1000
        testcase_run_1["stat"] = 2000
        ...
        six.assertCountEqual(self, self.mock.write_data.call_args_list, expected_calls)
    ...
    def test_upload_job_run(self):
        ...
        fuzzer_run = fuzzer_stats.JobRun(
            "fuzzer", "job", 123, 1472846341.017923, 9001, 0, 1, crashes
        )
    ...
    def test_testcase_run_read_from_disk(self, mock_path_exists):
        ...
        read_data = (
            '{"stat": 1000, "timestamp": 1472846341.017923, '
    ...
    def test_fuzz_task_upload_testcase_run_stats_builtin_fuzzer(
        ...
        testcase_run = fuzzer_stats.TestcaseRun(
            "placeholder", "placeholder", 0, 1472846341.017923
        )
    ...
    def test_fuzz_task_upload_job_run_stats(self):
        ...
        fuzz_task.upload_job_run_stats(
            "fuzzer", "job", 123, 1472846341.017923, 1, 2, 1337, groups
        )
        self.assertEqual(1, self.mock.write_data.call_count)
        self.assertEqual(
            {
...
class BuiltinFieldTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cov_info = data_types.CoverageInformation(fuzzer="fuzzer1", date=self.yesterday)
        ...
        cov_info = data_types.CoverageInformation(fuzzer="fuzzer2", date=self.yesterday)
        ...
        data_types.Job(
            name="job1", environment_string="FUZZ_LOGS_BUCKET = bucket1"
        ).put()
        data_types.Job(
            name="job2", environment_string="FUZZ_LOGS_BUCKET = bucket2"
        ).put()
    ...
    def test_coverage_fields(self):
        ...
        data = edge_field.get(fuzzer_stats.QueryGroupBy.GROUP_BY_DAY, self.yesterday)
    ...
    def test_corpus_size_fields(self):
        ...
        ctx = fuzzer_stats.CoverageFieldContext()
        corpus_field = fuzzer_stats.BuiltinFieldSpecifier("_CORPUS_SIZE").create(ctx)
        ...
        data = corpus_field.get(fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer1")
        self.assertEqual(data.value, "20 (200 B)")
        self.assertEqual(data.sort_key, 20)
        self.assertEqual(data.link, "gs://corpus")

        data = corpus_backup_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer1"
        )
        self.assertEqual(data.value, "Download")
        self.assertEqual(data.sort_key, None)
        self.assertEqual(data.link, "gs://corpus-backup")

        data = corpus_field.get(fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer2")
        self.assertEqual(data.value, "40 (99 B)")
        self.assertEqual(data.sort_key, 40)
        self.assertEqual(data.link, "gs://corpus")

        data = corpus_backup_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer2"
        )
        self.assertEqual(data.value, "Download")
        self.assertEqual(data.sort_key, None)
        self.assertEqual(data.link, "gs://corpus-backup")

        data = quarantine_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer1"
        )
        ...
        data = corpus_backup_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_DAY, self.today
        )
        self.assertEqual(data.value, "Download")
        self.assertEqual(data.sort_key, None)
        self.assertEqual(data.link, "gs://corpus-backup")

        data = corpus_field.get(fuzzer_stats.QueryGroupBy.GROUP_BY_DAY, self.yesterday)
        self.assertEqual(data.value, "15 (230 B)")
        self.assertEqual(data.sort_key, 15)
        self.assertEqual(data.link, "gs://corpus")

        data = corpus_backup_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_DAY, self.yesterday
        )
        self.assertEqual(data.value, "Download")
        self.assertEqual(data.sort_key, None)
        self.assertEqual(data.link, "gs://corpus-backup")
    ...
    def test_coverage_report_field(self):
        ...
        data = coverage_report_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_DAY, self.today
        )
        self.assertEqual(data.value, "Coverage")
        self.assertEqual(
            data.link,
            "https://report_for_fuzzer2/{}".format(
                data_types.coverage_information_date_to_string(self.today)
            ),
        )
    ...
    def test_logs_field_by_fuzzer(self):
        ...
        logs_field = fuzzer_stats.BuiltinFieldSpecifier("_FUZZER_RUN_LOGS").create(ctx)

        data = logs_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer_child1"
        )
    ...
    def test_logs_field_by_day(self):
        ...
        data = logs_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_DAY, datetime.date(2016, 11, 18)
        )
        self.assertEqual(data.value, "Logs")
    ...
    def test_performance_field_by_fuzzer(self):
        ...
        ctx = fuzzer_stats.FuzzerRunLogsContext("fuzzer1", ["job1"])
        performance_field = fuzzer_stats.BuiltinFieldSpecifier(
            "_PERFORMANCE_REPORT"
        ).create(ctx)

        data = performance_field.get(
            fuzzer_stats.QueryGroupBy.GROUP_BY_FUZZER, "fuzzer_child1"
        )
