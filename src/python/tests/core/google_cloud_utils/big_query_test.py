...
class InsertTest(unittest.TestCase):
    ...
    def test_insert(self):
        ...
        self.assertDictEqual(
            ...
            client.insert(
                [
                    big_query.Insert({"a": 1}, "prefix:0"),
                    big_query.Insert({"b": 2}, "prefix:1"),
                ]
            ),
        )
        tabledata.insertAll.assert_called_once_with(
            ...
            body={
                ...
                "rows": [
                    {"json": {"a": 1}, "insertId": "prefix:0"},
                    {"json": {"b": 2}, "insertId": "prefix:1"},
                ],
            },
        )
...
class ConvertTest(unittest.TestCase):
    ...
    def test_convert(self):
        ...
        fields = [
            ...
            {"name": "float", "type": "FLOAT", "mode": "REQUIRED"},
            {"name": "bool", "type": "BOOLEAN", "mode": "NULLABLE"},
            {"name": "string", "type": "STRING", "mode": "REQUIRED"},
            {"name": "list-string", "type": "STRING", "mode": "REPEATED"},
            {"name": "time", "type": "TIMESTAMP", "mode": "REQUIRED"},
            {
                ...
                "mode": "REPEATED",
                "fields": [
                    {"name": "name", "type": "STRING", "mode": u"NULLABLE"},
                    {
                        "fields": [
                            {"name": "hour", "type": "INTEGER", "mode": "NULLABLE"},
                            {"name": "count", "type": "INTEGER", "mode": u"NULLABLE"},
                        ],
                        "type": "RECORD",
                        "name": "slots",
                        "mode": "REPEATED",
            ...
            [
                {
                    ...
                    "string": "str",
                    "list-string": ["a", "b"],
                    "time": datetime.datetime.utcfromtimestamp(1485454187.696014),
                    "counts": [
                        {
                            "name": "fuzzer1",
                            "slots": [
...
class QueryTest(unittest.TestCase):
    ...
    def _make_resp(self, page_token, is_complete, total_row, count=1):
        ...
        resp = {
            "schema": {
                "fields": [{"name": "t", "type": "INTEGER", "mode": "NULLABLE"}]
            },
    ...
    def test_multiple_page_with_limit(self):
        ...
        result = client.query("sql", timeout=10, max_results=100, offset=50, limit=10)
    ...
    def test_timeout(self):
        ...
        self.assertEqual(
            "Timeout: the query doesn't finish within 10 seconds.", str(cm.exception)
        )
