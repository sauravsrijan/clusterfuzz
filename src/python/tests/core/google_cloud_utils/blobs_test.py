...
class BlobsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        helpers.patch(
            ...
            [
                ...
                "google_cloud_utils.blobs.generate_new_blob_name",
                "google_cloud_utils.storage.copy_file_from",
                "google_cloud_utils.storage.copy_file_to",
                "google_cloud_utils.storage.delete",
        ...
        blobs.BlobInfo(
            ...
            size=123,
            gs_object_name="/blobs-bucket/legacy",
        ).put()

        self.mock.get.return_value = {
            "metadata": {"filename": "gcs-file",},
            "size": 456,
        }
    ...
    def test_get_gcs_path(self):
        """Test get_gcs_path."""
        self.assertEqual("/blobs-bucket/" + TEST_UUID, blobs.get_gcs_path(TEST_UUID))

    def test_get_gcs_path_legacy(self):
        """Test get_gcs_path for legacy blobs."""
        self.assertEqual("/blobs-bucket/legacy", blobs.get_gcs_path("legacyblobkey"))
    ...
    def test_write_blob_file(self):
        ...
        self.mock.copy_file_to.assert_has_calls(
            [
                mock.call(
                    "/file", "/blobs-bucket/new-key", metadata={"filename": "file",}
                ),
            ]
        )
    ...
    def test_write_blob_handle(self):
        ...
        self.mock.copy_file_to.assert_has_calls(
            [
                mock.call(
                    handle, "/blobs-bucket/new-key", metadata={"filename": "filename",}
                ),
            ]
        )
