    ...
    def test_delete_with_age_and_new_versions(self):
        ...
        expected_config = {
            "rule": [
                {
                    "action": {"type": "Delete",},
                    "condition": {"age": 30, "numNewerVersions": 10,},
                }
            ],
        }
        self.assertEqual(config, expected_config)
...
class FileSystemProviderTests(fake_filesystem_unittest.TestCase):
    ...
    def test_get_bucket(self):
        ...
        self.assertDictEqual(
            {"name": "test-bucket",}, self.provider.get_bucket("test-bucket")
        )
    ...
    def test_list_blobs(self):
        ...
        self.fs.create_file(
            "/local/test-bucket/objects/a", st_size=11
        ).st_mtime = mtime_seconds
        self.fs.create_file(
            "/local/test-bucket/objects/b/c", st_size=22
        ).st_mtime = mtime_seconds
        self.fs.create_file(
            "/local/test-bucket/objects/b/d/e", st_size=33
        ).st_mtime = mtime_seconds
        self.fs.create_file(
            "/local/test-bucket/objects/f", st_size=44
        ).st_mtime = mtime_seconds
        self.fs.create_file(
            "/local/test-bucket/metadata/b/c", contents='{"key":"value"}'
        ).st_mtime = mtime_seconds
        ...
        result = list(self.provider.list_blobs("gs://test-bucket/", recursive=False))
        six.assertCountEqual(
            self,
            [
                {
    ...
    def test_get(self):
        ...
        self.fs.create_file(
            "/local/test-bucket/objects/a", contents="a"
        ).st_mtime = mtime_seconds
        self.fs.create_file(
