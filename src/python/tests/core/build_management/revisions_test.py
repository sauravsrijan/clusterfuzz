...
class RevisionsTestcase(unittest.TestCase):
    ...
    def mock_get_url_content(url):
        ...
        for line in REVISION_VARS_URL.splitlines():
            ...
            revision_without_path_and_query = revision_without_path.split('?')[
                0]
            deps_file = 'deps_%s_%s.txt' % (
                prefix, revision_without_path_and_query)
    ...
    def test_get_component_range_list_chromium(
        ...
        result = revisions.get_component_range_list(
            336903, 336983, BASIC_JOB_TYPE)
    ...
    def test_find_min_revision_index(self):
        ...
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 1000), 0)
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 2000), 1)
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 3000), 2)
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 4000), 3)

        self.assertIsNone(revisions.find_min_revision_index(revisions_list, 1))
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 1001), 0)
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 2001), 1)
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 3001), 2)
        self.assertEqual(revisions.find_min_revision_index(
            revisions_list, 4001), 3)

    def test_find_max_revision_index(self):
        """Tests find_max_revision_index()."""
        revisions_list = [1000, 2000, 3000, 4000]
        self.assertEqual(revisions.find_max_revision_index(
            revisions_list, 1000), 0)
        self.assertEqual(revisions.find_max_revision_index(
            revisions_list, 2000), 1)
        self.assertEqual(revisions.find_max_revision_index(
        ...
        self.assertEqual(
            revisions.find_max_revision_index(revisions_list, 1), 0)
        self.assertEqual(revisions.find_max_revision_index(
            revisions_list, 1001), 1)
        self.assertEqual(revisions.find_max_revision_index(
            revisions_list, 2001), 2)
        self.assertEqual(revisions.find_max_revision_index(
            revisions_list, 3001), 3)
        self.assertIsNone(
            revisions.find_max_revision_index(revisions_list, 4001))
