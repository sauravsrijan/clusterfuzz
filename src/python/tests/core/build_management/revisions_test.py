...
class RevisionsTestcase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock.ProjectConfig.return_value = mock_config.MockConfig(
            {
                "env": {
                    "REVISION_VARS_URL": "https://chromium.googlesource.com/chromium/src/+/%s/DEPS"
                    "?format=text"
                }
            }
        )
        ...
        data_types.Job(
            name=CUSTOM_BINARY_JOB_TYPE, environment_string=("CUSTOM_BINARY = True\n")
        ).put()
    ...
    def test_get_component_range_list_clank(
        ...
        result = revisions.get_component_range_list(260548, 260552, ANDROID_JOB_TYPE)
        result_as_html = revisions.format_revision_list(result)
    ...
    @mock.patch("config.db_config.get")
    @mock.patch("build_management.revisions._get_url_content")
    def test_get_git_hash_for_git_commit_pos(
    ...
    def test_get_real_revision_chromium(
        self, mock_get_git_hash, mock_get_url_content, mock_get_config
    ):
        ...
        self.assertEqual(
            "1d783bc2a3629b94c963debfa3feaee27092dd92",
            revisions.get_real_revision(336903, BASIC_JOB_TYPE),
        )

    @mock.patch("config.db_config.get")
    @mock.patch("build_management.revisions._get_url_content")
    def test_get_real_revision_oss_fuzz(self, mock_get_url_content, mock_get_config):
        ...
        self.assertEqual(
            "54444451414e3efb4a4c7e319a3f4110e20c7cf2",
            revisions.get_real_revision(9003, SRCMAP_JOB_TYPE),
        )

    @mock.patch("config.db_config.get")
    @mock.patch("build_management.revisions._get_url_content")
    def test_get_component_revision_list_src_map(
        self, mock_get_url_content, mock_get_config
    ):
    ...
    @mock.patch("config.db_config.get")
    @mock.patch("build_management.revisions._get_url_content")
    def test_get_component_revision_list_src_map_text(
        self, mock_get_url_content, mock_get_config
    ):
    ...
    @mock.patch("config.db_config.get")
    @mock.patch("build_management.revisions._get_url_content")
    def test_get_component_range_list_same_hash(
        self, mock_get_url_content, mock_get_config
    ):
    ...
    @mock.patch("config.db_config.get")
    @mock.patch("build_management.revisions._get_url_content")
    def test_get_component_range_list_0_start_custom(
        self, mock_get_url_content, mock_get_config
    ):
    ...
    def test_get_component_range_list_custom_binary_job(
        ...
        self.assertEqual(
            [], revisions.get_component_range_list(1337, 1338, CUSTOM_BINARY_JOB_TYPE)
        )
