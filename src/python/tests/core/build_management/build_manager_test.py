...
def _mock_unpack_build(self, _, build_dir, build_url, target_weights=None):
    """Mock _unpack_build."""
...
class TrunkBuildTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ["RELEASE_BUILD_BUCKET_PATH"] = "gs://path/file-release-([0-9]+).zip"
        os.environ[
...
class RegularBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def test_delete(self):
        ...
        self.mock.get_build_urls_list.return_value = ["gs://path/file-release-2.zip"]
...
class RegularLibFuzzerBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.target_weights = {"target1": 0.0, "target2": 1.0, "target3": 0.0}
    ...
    def test_setup_fuzz(self, unpack_all):
        ...
        self.mock.time.return_value = 1000.0
        build = build_manager.setup_regular_build(2, target_weights=self.target_weights)
        self.assertIsInstance(build, build_manager.RegularBuild)
        self.assertEqual(_get_timestamp(build.base_build_dir), 1000.0)
        ...
        self.mock.time.return_value = 1005.0
        self.assertIsInstance(
            build_manager.setup_regular_build(2, target_weights=self.target_weights),
...
class SymbolizedBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def _assert_env_vars_both(self):
        ...
        self.assertEqual(
            os.environ["APP_DIR"],
            "/builds/path_be4c9ca0267afcd38b7c1a3eebb5998d0908f025/symbolized/" "debug",
        )
...
class ProductionBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ["STABLE_BUILD_BUCKET_PATH"] = "gs://path/file-stable-([0-9.]+).zip"
    ...
    def _assert_env_vars(self, build_type):
        """Assert env vars."""
        self.assertEqual(
            os.environ["BUILD_URL"], "gs://path/file-%s-45.0.1824.2.zip" % build_type
        )

        self.assertEqual(
            os.environ["APP_PATH"],
            "/builds/path_8102046d3cea496c945743eb5f79284e7b10b51b/%s/app" % build_type,
        )
...
class CustomBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def test_setup(self):
        ...
        self.mock.time.return_value = 1005.0
        self.assertIsInstance(
            build_manager.setup_custom_binary(), build_manager.CustomBuild
        )
        self.assertEqual(_get_timestamp(build.base_build_dir), 1005.0)
...
class SystemBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        _mock_unpack_build(build_manager.SystemBuild("/"), None, "/system_binary", None)
...
class AuxiliaryRegularBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def _assert_env_vars(self):
        ...
        self.assertEqual(
            os.environ["DATAFLOW_BUILD_URL"], "gs://path/file-dataflow-10.zip"
        )
    ...
    def test_delete(self):
        ...
        self.mock.get_build_urls_list.return_value = ["gs://path/file-dataflow-2.zip"]
...
class BuildEvictionTests(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fs.create_file("/builds/build1/.timestamp", contents="1486166114.668105")
        self.fs.create_file("/builds/build2/.timestamp", contents="1486166110.142942")
        self.fs.create_file("/builds/build3/.timestamp", contents="1486166112.180345")
    ...
    def test_make_space_for_build_remove_one_build(self):
        ...
        self.free_disk_space = [9 * 1024 * 1024 * 1024, 24 * 1024 * 1024 * 1024]
    ...
    def test_make_space_for_build_no_builds_to_remove(self):
        ...
        self.mock.extracted_size.return_value = 20 * 1024 * 1024 * 1024  # 1 GB
        self.mock.get_free_disk_space.side_effect = self._mock_free_disk_space
        self.free_disk_space = [18 * 1024 * 1024 * 1024]
...
class GetFileMatchCallbackTest(unittest.TestCase):
    ...
    def _test_helper(self, fuzz_target):
        ...
        self.assertTrue(
            match_callback("build/instrumented_libraries/msan/lib/libgcrypt.so.11.8.2")
        )
        ...
        self.assertFalse(
            match_callback(
                r"/b/build/other_fuzzer.dSYM/Contents/Resources/DWARF/"
                "some_dependency"
            )
        )
...
class RpathsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ[
            "INSTRUMENTED_LIBRARIES_PATHS_MSAN_CHAINED"
        ] = "/msan/lib:/msan/usr/lib"
        os.environ[
            "INSTRUMENTED_LIBRARIES_PATHS_MSAN_NO_ORIGINS"
    ...
    def test_patch_rpaths_no_origins(self):
        ...
        self.assertListEqual(
            ["/msan-no-origins/lib", "/msan-no-origins/usr/lib"], rpaths
        )
    ...
    def test_patch_rpaths_not_available(self):
        ...
        build = build_manager.RegularBuild(self.base_build_dir, 1337, "chained-origins")
        self.assertTrue(build.setup())
...
class SortBuildUrlsByRevisionTest(unittest.TestCase):
    ...
    def test_bucket_root(self):
        ...
        bucket_path = (
            "gs://chromium-browser-libfuzzer/" "libfuzzer-linux-release-([0-9]+).zip"
        )
        build_urls = [
...
class SplitFuzzTargetsBuildTest(fake_filesystem_unittest.TestCase):
    ...
    def _assert_env_vars(self, target, revision):
        ...
        self.assertEqual(
            ...
            "77651789446b3c3a04b9f492ff141f003d437347/revisions".format(target=target),
            os.environ["BUILD_DIR"],
        )
...
class GetPrimaryBucketPathTest(unittest.TestCase):
    ...
    def test_release_bucket_path(self):
        ...
        self.assertEqual("gs://release_build", build_manager.get_primary_bucket_path())
    ...
    def test_fuzz_target_bucket_path(self):
        ...
        self.assertEqual(
            "gs://fuzz_target/test_target/path", build_manager.get_primary_bucket_path()
        )
