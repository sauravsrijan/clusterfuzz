...
class TrunkBuildTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    os.environ[
        "RELEASE_BUILD_BUCKET_PATH"] = "gs://path/file-release-([0-9]+).zip"
    os.environ[
        "SYM_RELEASE_BUILD_BUCKET_PATH"] = "gs://path/file-sym-release-([0-9]+).zip"
    os.environ[
        "SYM_DEBUG_BUILD_BUCKET_PATH"] = "gs://path/file-sym-debug-([0-9]+).zip"
...
class SymbolizedBuildTest(fake_filesystem_unittest.TestCase):
  ...
  def _prepare_test(self, release_urls, debug_urls):
    ...
    if release_urls:
      os.environ[
          "SYM_RELEASE_BUILD_BUCKET_PATH"] = "gs://path/file-release-([0-9]+).zip"

    if debug_urls:
      os.environ[
          "SYM_DEBUG_BUILD_BUCKET_PATH"] = "gs://path/file-debug-([0-9]+).zip"
...
class ProductionBuildTest(fake_filesystem_unittest.TestCase):
  ...
  def setUp(self):
    ...
    os.environ[
        "STABLE_BUILD_BUCKET_PATH"] = "gs://path/file-stable-([0-9.]+).zip"
...
class AuxiliaryRegularBuildTest(fake_filesystem_unittest.TestCase):
  ...
  def test_setup(self):
    """Tests setting up a build."""
    os.environ[
        "DATAFLOW_BUILD_BUCKET_PATH"] = "gs://path/file-dataflow-([0-9]+).zip"
...
class RpathsTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    os.environ[
        "INSTRUMENTED_LIBRARIES_PATHS_MSAN_CHAINED"] = "/msan/lib:/msan/usr/lib"
    os.environ[
        "INSTRUMENTED_LIBRARIES_PATHS_MSAN_NO_ORIGINS"] = "/msan-no-origins/lib:/msan-no-origins/usr/lib"
    os.environ["FAIL_RETRIES"] = "1"
...
class SplitFuzzTargetsBuildTest(fake_filesystem_unittest.TestCase):
  ...
  def setUp(self):
    ...
    os.environ[
        "FUZZ_TARGET_BUILD_BUCKET_PATH"] = "gs://bucket/subdir/%TARGET%/([0-9]+).zip"
