...
def mock_popen_factory(execute_time, output, sigterm_handler_time, return_code=0):
    """Create a mock popen."""
        ...
        def communicate(self, input_data=None):
            ...
            for i in range(2):
                ...
                self.received_signals.append(
                    (received_signal, time.time() - self.start_time)
                )
...
class PosixProcessTest(unittest.TestCase):
    ...
    def test_results_no_timeout(self):
        ...
        with mock.patch("subprocess.Popen", mock_popen_factory(1.0, "output", 0.0, 0)):
            runner = new_process.ProcessRunner(
                "/test/path", default_args=["-arg1", "-arg2"]
            )
    ...
    def test_timeout(self):
        ...
        with mock.patch(
            "subprocess.Popen", mock_popen_factory(1.0, "", 0.0)
        ) as mock_popen:
            runner = new_process.ProcessRunner("/test/path")
    ...
    def test_no_timeout(self):
        ...
        with mock.patch(
            "subprocess.Popen", mock_popen_factory(0.5, "", 0.0)
        ) as mock_popen:
            runner = new_process.ProcessRunner("/test/path")
    ...
    def test_terminate_before_kill_no_sigterm_timeout(self):
        ...
        with mock.patch(
            "subprocess.Popen", mock_popen_factory(1.0, "", 0.5)
        ) as mock_popen:
            runner = new_process.ProcessRunner("/test/path")
            runner.run_and_wait(
    ...
    def test_terminate_before_kill_timeout(self):
        ...
        with mock.patch(
            "subprocess.Popen", mock_popen_factory(1.0, "", 1.0)
        ) as mock_popen:
            runner = new_process.ProcessRunner("/test/path")
            runner.run_and_wait(
