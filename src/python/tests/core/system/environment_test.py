...
class GetExecutableFileNameTest(unittest.TestCase):
    ...
    def test_non_windows(self, platform):
        ...
        self.assertEqual(
            self.EXECUTABLE, environment.get_executable_filename(self.EXECUTABLE)
        )
...
class ParseMemoryToolOptionsTest(unittest.TestCase):
    ...
    def setUp(self):
        test_helpers.patch(self, ["metrics.logs.log_warn", "metrics.logs.log_error"])
        ...
        self.windows_expected = {
            ...
            "external_symbolizer_path": r'"c:\clusterfuzz\resources\platform\windows\llvm-symbolizer.exe"',
            "handle_sigill": 1,
    ...
    def _helper(self, options_str, expected):
        ...
        had |expected| result."""
        parsed_options = environment._parse_memory_tool_options(
            options_str
...
class EvalValueTest(unittest.TestCase):
    ...
    def test_eval_value(self, value_string, expected_result):
        ...
        actual_result = environment._eval_value(
            value_string
        )  # pylint: disable=protected-access
...
class ResetCurrentMemoryToolOptionsTest(unittest.TestCase):
    ...
    def test_ubsan_disabled(self):
        ...
        self.assertDictEqual(
            {"halt_on_error": 0, "print_stacktrace": 0, "print_suppressions": 0},
            environment.get_memory_tool_options("UBSAN_OPTIONS"),
        )
...
class MaybeConvertToIntTest(unittest.TestCase):
    ...
    def test_maybe_convert_to_int(self, literal_value, expected_result):
        ...
        result."""
        actual_result = environment._maybe_convert_to_int(
            literal_value
        )  # pylint: disable=protected-access
        self.assertEqual(expected_result, actual_result)
...
class GetMemoryToolOptionsTest(unittest.TestCase):
    ...
    def test_doesnt_mutate_options(self):
        ...
        first_asan_options_dict = environment.get_memory_tool_options(memory_tool_var)
        environment.set_memory_tool_options(memory_tool_var, first_asan_options_dict)
        second_asan_options_dict = environment.get_memory_tool_options(memory_tool_var)
