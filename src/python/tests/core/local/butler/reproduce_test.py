...
class DownloadTestcaseTest(unittest.TestCase):
    ...
    def test_non_archived_testcase(self):
        ...
        testcase = reproduce.SerializedTestcase(
            {"archive_state": data_types.ArchiveStatus.NONE, "minimized_keys": "key"}
        )

        reproduce._download_testcase(1, testcase, self.config)
        self.mock.write_data_to_file.assert_called_once_with(
            "html data",
            os.path.join(reproduce.CONFIG_DIRECTORY, "current-testcase", "test.html"),
        )
    ...
    def test_archived_testcase(self):
        ...
        current_testcase_directory = os.path.join(
            reproduce.CONFIG_DIRECTORY, "current-testcase"
        )
        ...
        self.mock.unpack.assert_called_once_with(zip_path, current_testcase_directory)
