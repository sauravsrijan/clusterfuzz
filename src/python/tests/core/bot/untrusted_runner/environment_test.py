...
@mock.patch(
    "bot.untrusted_runner.environment.FORWARDED_ENVIRONMENT_VARIABLES",
    FORWARDED_ENVIRONMENT_VARIABLES,
)
@mock.patch(
    "bot.untrusted_runner.environment.REBASED_ENVIRONMENT_VARIABLES",
    REBASED_ENVIRONMENT_VARIABLES,
)
class EnvironmentTest(unittest.TestCase):
    ...
    def setUp(self):
        helpers.patch(self, ["bot.untrusted_runner.host.stub",])
    ...
    def test_is_forwarded_environment_variable(self):
        ...
        self.assertTrue(environment.is_forwarded_environment_variable("ASAN_OPTIONS"))
        self.assertTrue(environment.is_forwarded_environment_variable("REBASED"))
        self.assertFalse(environment.is_forwarded_environment_variable("FUZZER_DIR"))
        self.assertFalse(environment.is_forwarded_environment_variable("ASAN_OPTIONSS"))
    ...
    def test_should_rebase_environment_value(self):
        ...
        self.assertFalse(environment.should_rebase_environment_value("ASAN_OPTIONS"))
    ...
    def test_set_environment_vars(self):
        ...
        self.assertDictEqual(
            {"ASAN_OPTIONS": "options", "REBASED": "/worker/rebased",}, result
        )
