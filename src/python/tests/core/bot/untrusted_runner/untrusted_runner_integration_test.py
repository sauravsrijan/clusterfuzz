...
TEST_FILE_CONTENTS = (
    b"A" * config.FILE_TRANSFER_CHUNK_SIZE
...
def _dirs_equal(dircmp):
    ...
    return all(_dirs_equal(sub_dircmp) for sub_dircmp in six.itervalues(dircmp.subdirs))
...
class UntrustedRunnerIntegrationTest(
    ...
    def setUp(self):
        ...
        data_types.Job(
            name="libfuzzer_asan_job", environment_string=environment_string
        ).put()
    ...
    def test_run_process(self):
        ...
        return_code, _, output = remote_process_host.run_process(
            "ls", current_working_directory="/"
        )
    ...
    def test_run_process_env(self):
        ...
        result = runner.run_and_wait(
            ...
            env={"UBSAN_OPTIONS": "ubsan", "NOT_PASSED": "blah"},
        )
        ...
        _, _, output = process_handler.run_process(
            ...
            testcase_run=True,
            env_copy={"MSAN_OPTIONS": "msan", "NOT_PASSED": "blah"},
        )
    ...
    def test_setup_regular_build(self):
        ...
        expected_build_dir = os.path.join(
            ...
            "bot",
            "builds",
            "clusterfuzz-test-data_test_builds_"
            "2b6ddd7575e9b06b20306183720c65fff3ce318d",
            "revisions",
        )
        ...
        self.assertEqual(os.path.join(expected_app_dir, "app"), os.environ["APP_PATH"])
    ...
    def test_setup_regular_build_fuzz_target(self):
        ...
        environment.set_value(
            "FUZZER_DIR", os.path.join(os.environ["ROOT_DIR"], launcher_dir)
        )
        ...
        expected_build_dir = os.path.join(
            ...
            "41a87efdd470c6f00e8babf61548bf6c7de57137",
            "revisions",
        )
    ...
    def test_setup_testcase(self):
        ...
        six.assertCountEqual(self, file_list, [testcase.absolute_path,])
    ...
    def test_get_command_line_for_application(self):
        ...
        launcher_path = os.path.join(os.environ["FUZZERS_DIR"], "test", "launcher.py")
    ...
    def test_corpus_sync(self):
        ...
        corpus = corpus_manager.RemoteFuzzTargetCorpus(
            "corpus_test_fuzzer", "child_fuzzer"
        )
        ...
        try:
            ...
            six.assertCountEqual(
                self, os.listdir(test_corpus_directory), ["123", "456", "abc"]
            )
        finally:
    ...
    def test_large_message(self):
        ...
        result = runner.run_and_wait(additional_args=["-c", 'print "A"*5*1024*1024'])
        self.assertEqual(result.return_code, 0)
        self.assertEqual(result.output, expected_output)
