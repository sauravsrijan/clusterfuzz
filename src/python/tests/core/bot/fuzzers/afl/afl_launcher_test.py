...
class LauncherTestBase(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_helpers.patch(self, ["bot.fuzzers.utils.get_temp_dir",])
...
class FuzzingStrategiesTest(fake_filesystem_unittest.TestCase):
    ...
    def test_decide_fast_cal_random_finds_correct_prob(self):
        ...
        test_helpers.patch(self, ["bot.fuzzers.engine_common.decide_with_probability",])
...
class AflFuzzInputDirectoryTest(LauncherTestBase):
    ...
    def _new_afl_input(self):
        ...
        return launcher.AflFuzzInputDirectory(
            self.INPUT_DIR, self.TARGET_PATH, self.strategies
        )
...
class AflFuzzOutputDirectoryTest(LauncherTestBase):
    ...
    def test_copy_crashes_if_needed(self):
        ...
        crash_path_1 = os.path.join(
            self.CRASHES_DIR, "id:000012,src:000000,op:flip1,pos:1,+cov"
        )
        ...
        crash_path_2 = os.path.join(
            self.CRASHES_DIR, "id:000031,src:000000,op:arith8,pos:0,val:+7"
        )
...
class SetSanitizerOptionsTest(LauncherTestBase):
    ...
    def assert_sanitizer_opts_set(
        self, sanitizer_options_variable, *additional_options
    ):
    ...
    def test_options_file(self):
        ...
        launcher.set_additional_sanitizer_options_for_afl_fuzz()
        self.assert_sanitizer_opts_set(
            "ASAN_OPTIONS", self.DUMMY_OPTION, "fake_option=1"
        )
        self.assert_sanitizer_opts_set(
            "MSAN_OPTIONS", self.DUMMY_OPTION, "fake_option=2"
        )
...
class AflRunnerTest(LauncherTestBase):
    ...
    def setUp(self):
        ...
        self.runner = launcher.AflRunner(
            self.TARGET_PATH, config, self.TESTCASE_FILE_PATH, self.INPUT_DIR
        )
    ...
    def test_gen_afl_args(self):
        ...
        runner = launcher.AflRunner(target_path, config, testcase_file_path, input_dir)
    ...
    def test_set_environment_variables(self):
        ...
        self.runner.afl_setup()
        self.assertEqual(
            os.environ["AFL_DRIVER_STDERR_DUPLICATE_FILENAME"],
    ...
    def test_set_arg(self):
        ...
        self.assertEqual(self.args, ["-xdict", "-iinput2", "-ooutput", "123", "456"])
    ...
    def test_fuzzer_stderr(self):
        ...
        self.assertLess(len(self.runner.fuzzer_stderr), launcher.MAX_OUTPUT_LEN + 50)
    ...
    def _initialization_for_run_afl(self):
        ...
        test_helpers.patch(self, ["bot.fuzzers.afl.launcher.AflRunner.run_and_wait",])
    ...
    def test_run_afl_fuzz_crashing_input(self):
        ...
        self.assertEqual(
            self.runner.run_afl_fuzz(self.args).return_code, self.ARBITRARY_RET_CODE
        )
...
class GetFuzzTimeoutTest(GetTimeoutTestBase):
    ...
    function = staticmethod(partial(launcher.get_fuzz_timeout, is_mutations_run=False))
    ...
    def test_correctness(self):
        ...
        expected_fuzz_timeout = (
            ...
            - engine_common.POSTPROCESSING_TIME
...
class AflConfigTest(LauncherTestBase):
    ...
    def test_libfuzzer_section_dict(self):
        ...
        self.assertIn(
            "-x/build_dir/my_dict.dict", self._get_config().additional_afl_arguments
        )
...
class CorpusTest(fake_filesystem_unittest.TestCase):
    ...
    def test_associate_new_features_with_file(self):
        ...
        for feature in features:
            self.assertEqual(filename, self.corpus.features_and_elements[feature].path)
    ...
    def test_associate_feature_with_smaller_file(self):
        ...
        self.assertEqual(
            smaller_filename, self.corpus.features_and_elements[features[0]].path
        )
    ...
    def test_file_with_one_feature_remains(self):
        ...
        larger_filename = "larger"
        self._create_file(larger_filename, size=2)
        self.corpus.associate_features_with_file(
            [feature_1, feature_2], larger_filename
        )
        smaller_filename = "smaller"
        self._create_file(smaller_filename, size=1)
        self.corpus.associate_features_with_file([feature_2], smaller_filename)
        self.assertEqual(
            smaller_filename, self.corpus.features_and_elements[feature_2].path
        )
        self.assertEqual(
            larger_filename, self.corpus.features_and_elements[feature_1].path
        )
        self.assertEqual({smaller_filename, larger_filename}, self.corpus.element_paths)
