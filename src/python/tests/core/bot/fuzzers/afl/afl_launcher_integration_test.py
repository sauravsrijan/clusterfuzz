...
# behind a flag.
if environment.get_value("AFL_INTEGRATION_TESTS") and not environment.get_value(
    "TEST_BOT_ENVIRONMENT"
):
...
def get_fuzz_timeout(fuzz_time):
    ...
    return (
        fuzz_time
        + launcher.AflRunner.AFL_CLEAN_EXIT_TIME
...
def setup_testcase_and_corpus(testcase, corpus, fuzz=False):
    ...
    if os.path.exists(src_corpus_path):
        ...
        with open(os.path.join(copied_corpus_path, fuzzer.AFL_DUMMY_INPUT), "w") as f:
            f.write(" ")
...
def mocked_is_testcase(path):
    ...
    return os.path.isfile(path) and bool(
        re.match(testcase_regex, os.path.basename(path))
    )
...
@unittest.skipIf(
    not environment.get_value("AFL_INTEGRATION_TESTS"),
...
class BaseLauncherTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_helpers.patch(self, ["metrics.logs.log", "os.getpid"])
    ...
    def _test_libfuzzerize_corpus(self, mock_get_timeout):
        ...
        testcase_path = setup_testcase_and_corpus("empty", "input_corpus", fuzz=True)
...
class TestLauncher(BaseLauncherTest):
    ...
    def test_single_testcase_crash(self):
        ...
        self.assertIn(
            "ERROR: AddressSanitizer: SEGV on unknown address 0x000000000000", output
        )
    ...
    def test_fuzz_input_crash(self, mock_get_timeout):
        ...
        self.assertIn(
            "Command: {0}/afl-fuzz -i{1}/corpus "
            "-o{1}/temp-1337/afl_output_dir -mnone "
            "{0}/always_crash_fuzzer 2147483647".format(DATA_DIRECTORY, TEMP_DIRECTORY),
            output,
        )
    ...
    def test_fuzz_crash(self, mock_get_timeout):
        ...
        self.assertIn(
            "Command: {0}/afl-fuzz -i{1}/corpus "
            "-o{1}/temp-1337/afl_output_dir -mnone "
            "{0}/easy_crash_fuzzer 2147483647".format(DATA_DIRECTORY, TEMP_DIRECTORY),
            output,
        )

        self.assertIn("ERROR: AddressSanitizer: heap-use-after-free on address", output)
...
def _mknod(_, path, file_type, major, minor):
    ...
    _run_with_sudo(["mknod", "-m", "666", path, file_type, str(major), str(minor)])
...
class TestLauncherMinijail(BaseLauncherTest):
    ...
    def test_fuzz_merge(self, mock_get_timeout):
        ...
        self.assertIn(
            ...
            "-o/afl_output_dir -mnone "
            "{0}/test_fuzzer 2147483647".format(DATA_DIRECTORY),
            output,
        )
