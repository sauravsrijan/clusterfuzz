...
import six
from bot.fuzzers import builtin, builtin_fuzzers
from bot.tasks import fuzz_task, setup
...
class BuiltinFuzzersTest(unittest.TestCase):
    ...
    def test_get(self):
        """Tests get()."""
        self.assertIsInstance(builtin_fuzzers.get("libFuzzer"), builtin.BuiltinFuzzer)
...
class BuiltinFuzzersSetupTest(fake_filesystem_unittest.TestCase):
    ...
    def test_generate_blackbox_fuzzers(self):
        ...
        self.mock.run.side_effect = functools.partial(
            _mock_fuzzer_run, output, 4, "corpus_dir"
        )
        ...
        six.assertCountEqual(
            self,
            ["/output/fuzz-0", "/output/fuzz-1", "/output/fuzz-2", "/output/fuzz-3",],
            testcase_file_paths,
        )

        self.assertEqual("corpus_dir", sync_corpus_directory)
        self.assertDictEqual(
            {"fuzzer_binary_name": "fuzzer_binary_name"}, fuzzer_metadata
        )
