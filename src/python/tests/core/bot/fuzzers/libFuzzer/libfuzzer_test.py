    ...
    class MockPopen(object):
        ...
        def _do_merge(self):
            ...
            for filepath in os.listdir(corpus_path):
                shutil.copy(os.path.join(corpus_path, filepath),
                            merge_corpus_path)
...
class RecommendedDictionaryTest(fake_fs_unittest.TestCase):
    ...
    def setUp(self):
        ...
        dictionaries_directory = os.path.join(
            self.data_directory, 'dictionaries')
        ...
        self.fuzzer_path = os.path.join(
            self.work_directory_path, self.fuzzer_name)
    ...
    def test_add_recommended_dictionary_with_merge(self):
        ...
        updated_dictionary_data = read_data_from_file(
            dictionary_path + '.merged')
...
class MoveMergeableUnitsTest(fake_fs_unittest.TestCase):
    ...
    def move_mergeable_units(self):
        ...
        libfuzzer.move_mergeable_units(
            self.MERGE_DIRECTORY, self.CORPUS_DIRECTORY)
    ...
    def test_duplicate_not_moved(self):
        ...
        merge_corpus_file = os.path.join(
            self.MERGE_DIRECTORY, ARBITRARY_SHA1_HASH)
