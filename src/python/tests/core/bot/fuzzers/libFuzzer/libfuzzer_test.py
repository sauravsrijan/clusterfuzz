...
class RecommendedDictionaryTest(fake_fs_unittest.TestCase):
    ...
    def mock_download_recommended_dictionary_from_gcs(self, _, local_dictionary_path):
        """Mock for DictionaryManager.download_recommended_dictionary_from_gcs."""
        engine_common.write_data_to_file(
            self.fake_gcs_dictionary_data, local_dictionary_path
        )
        return True
    ...
    def setUp(self):
        ...
        self.expected_gcs_only_merged_dictionary_data = read_data_from_file(
            os.path.join(
                dictionaries_directory, "expected_gcs_only_merged_dictionary.dict"
            )
        )
        ...
        self.local_dictionaries_dir = os.path.join(self.work_directory_path, "dicts")
    ...
    def test_add_recommended_dictionary_no_merge(self):
        ...
        libfuzzer.add_recommended_dictionary(
            arguments, self.fuzzer_name, self.fuzzer_path
        )
    ...
    def test_add_recommended_dictionary_with_merge(self):
        ...
        dictionary_path = os.path.join(
            self.work_directory_path, "dictionary_from_repository.dict"
        )
        self.fs.create_file(
        ...
        libfuzzer.add_recommended_dictionary(
            actual_arguments, self.fuzzer_name, self.fuzzer_path
        )
        ...
        self.assert_compare_dictionaries(
            updated_dictionary_data, self.expected_merged_dictionary_data
        )
...
class MoveMergeableUnitsTest(fake_fs_unittest.TestCase):
    ...
    def test_duplicate_not_moved(self):
        ...
        self.fs.create_file(os.path.join(self.CORPUS_DIRECTORY, ARBITRARY_SHA1_HASH))
    ...
    def test_new_file_moved(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(self.CORPUS_DIRECTORY, filename)))
...
class SelectGeneratorTest(unittest.TestCase):
    ...
    def test_lpm_fuzz_target(self):
        self.assertEqual(
            engine_common.Generator.NONE,
            engine_common.select_generator(self.pool, self.FUZZER_PATH),
