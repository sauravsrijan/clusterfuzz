...
class PrepareTest(fake_fs_unittest.TestCase):
    ...
    def test_prepare(self):
        ...
        six.assertCountEqual(
            self, ["/new_corpus_dir", "/corpus_dir"], options.fuzz_corpus_dirs
        )
...
class UnshareIntegrationTests(IntegrationTests):
    ...
    def compare_arguments(self, target_path, arguments, corpora_or_testcase, actual):
        ...
        self.assertListEqual(
            actual,
            ["/usr/bin/unshare", "-n", target_path] + arguments + corpora_or_testcase,
        )
...
class IntegrationTestsFuchsia(BaseIntegrationTest):
    ...
    def test_fuzzer_can_boot_and_run_with_corpus(self):
        ...
        options = engine_impl.prepare(
            corpus_path, "example-fuzzers/trap_fuzzer", DATA_DIR
        )
        results = engine_impl.fuzz("example-fuzzers/trap_fuzzer", options, TEMP_DIR, 20)
    ...
    def test_minimize_testcase(self):
        ...
        environment.set_value("JOB_NAME", "libfuzzer_asan_fuchsia")
        build_manager.setup_build()
        testcase_path, _ = setup_testcase_and_corpus(
            "fuchsia_overlong_crash", "empty_corpus"
        )
        minimize_output_path = os.path.join(TEMP_DIR, "output")

        engine_impl = engine.LibFuzzerEngine()
        result = engine_impl.minimize_testcase(
            "example-fuzzers/trap_fuzzer",
            ["-runs=1000000"],
            testcase_path,
            minimize_output_path,
            30,
        )
        with open(minimize_output_path) as f:
            result = f.read()
