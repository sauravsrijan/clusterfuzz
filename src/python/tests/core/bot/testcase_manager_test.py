...
class CreateTestcaseListFileTest(fake_filesystem_unittest.TestCase):
    ...
    def test_create(self):
        ...
        expected_files_list = set(["bb.txt", "cc.txt", "aa/dd.txt"])
...
class UploadTestcaseOutputTest(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock.get_component_range_list.return_value = [
            {"component": "Component", "link_text": "REVISION"},
            {"component": "Component2", "link_text": "REVISION2"},
        ]
    ...
    def test_upload_with_timestamp_from_stats(self):
        ...
        crash_result = CrashResult(return_code=1, crash_time=5, output="fake output")
...
class ConvertDependencyUrlToLocalPathTest(unittest.TestCase):
    ...
    def test_url_match_any_ip_and_port(self):
        ...
        self.mock.get_absolute_testcase_file.assert_called_once_with("/test/test.html")
...
class GetResourcePathsTest(unittest.TestCase):
    ...
    def setUp(self):
        test_helpers.patch(
            self, ["bot.testcase_manager.convert_dependency_url_to_local_path"]
        )
...
class GetCrashOutputTest(unittest.TestCase):
    ...
    def test_none(self):
        self.assertEqual(
            None, testcase_manager._get_crash_output(None)
        )  # pylint: disable=protected-access
...
class TestcaseRunningTest(fake_filesystem_unittest.TestCase):
    ...
    def test_test_for_crash_with_retries_greybox_fail(self):
        ...
        mock_engine.reproduce.assert_has_calls(
            [
                mock.call(
                    "/build_dir/target", "/fuzz-testcase", ["-arg1", "-arg2"], 120
                ),
                mock.call(
                    "/build_dir/target", "/fuzz-testcase", ["-arg1", "-arg2"], 10
                ),
                mock.call(
                    "/build_dir/target", "/fuzz-testcase", ["-arg1", "-arg2"], 10
                ),
        ...
        self.mock.log.assert_has_calls(
            [
                mock.call(
                    "No crash occurred (round 1).", output=self.GREYBOX_FUZZER_NO_CRASH
                ),
                mock.call(
                    "No crash occurred (round 2).", output=self.GREYBOX_FUZZER_NO_CRASH
                ),
                mock.call(
                    "No crash occurred (round 3).", output=self.GREYBOX_FUZZER_NO_CRASH
                ),
                mock.call("Didn't crash at all."),
            ]
        )
    ...
    def test_test_for_crash_with_retries_blackbox_succeed(self):
        ...
        self.mock.run_process.side_effect = [(0, 0, "output"), (1, 1, "crash")]
    ...
    def test_test_for_crash_with_retries_greybox_legacy(self):
        ...
        mock_engine.reproduce.assert_has_calls(
            [mock.call("/build_dir/target", "/fuzz-testcase", ["-arg1", "-arg2"], 120)]
        )
        self.mock.log.assert_has_calls(
            [
                mock.call(
...
class UntrustedEngineReproduceTest(
    ...
    def test_reproduce(self):
        ...
        self.assertEqual(result.return_code, libfuzzer_constants.TARGET_ERROR_EXITCODE)
        self.assertGreater(result.time_executed, 0)
        self.assertIn("Running 1 inputs 100 time(s) each", result.output)
        self.assertIn(
            "AddressSanitizer: SEGV on unknown address 0x000000000000", result.output
        )
...
class GetCommandLineFlagsTest(fake_filesystem_unittest.TestCase):
    ...
    def test_both_args_and_additional(self):
        ...
        self.assertEqual(
            "arg1 arg2", testcase_manager.get_command_line_flags("/fuzz-testcase")
        )
    ...
    def test_only_args(self):
        ...
        os.environ["APP_ARGS"] = "arg1"
        self.assertEqual(
            "arg1", testcase_manager.get_command_line_flags("/fuzz-testcase")
        )
    ...
    def test_only_additional(self):
        ...
        self.fs.create_file("/flags-testcase", contents="arg2")
        self.assertEqual(
            "arg2", testcase_manager.get_command_line_flags("/fuzz-testcase")
        )

    def test_no_args_and_additional(self):
        """Test both APP_ARGS and additional args."""
        self.assertEqual("", testcase_manager.get_command_line_flags("/fuzz-testcase"))
