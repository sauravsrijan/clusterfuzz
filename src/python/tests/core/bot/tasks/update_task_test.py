...
class TrackRevisionTest(fake_filesystem_unittest.TestCase):
    ...
    def test_no_revision(self):
        ...
        self.assertEqual(0, monitoring_metrics.BOT_COUNT.get({"revision": "revision"}))
    ...
    def test_has_revision(self):
        ...
        update_task.track_revision()
        self.assertEqual(1, monitoring_metrics.BOT_COUNT.get({"revision": "revision"}))
...
class GetUrlsTest(unittest.TestCase):
    ...
    def setUp(self):
        helpers.patch_environ(self)
        helpers.patch(self, ["platform.system",])
        if sys.version_info.major == 3:
            self.manifest_suffix = ".3"
