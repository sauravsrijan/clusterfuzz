...
from bot.tasks import commands, corpus_pruning_task
...
class CorpusPruningTest(unittest.TestCase, BaseTest):
    ...
    def test_prune(self):
        """Basic pruning test."""
        corpus_pruning_task.execute_task("libFuzzer_test_fuzzer", "libfuzzer_asan_job")

        quarantined = os.listdir(self.quarantine_dir)
        self.assertEqual(1, len(quarantined))
        self.assertEqual(
            quarantined[0], "crash-7acd6a2b3fe3c5ec97fa37e5a980c106367491fa"
        )
        ...
        self.assertEqual("test_fuzzer", testcases[0].get_metadata("fuzzer_binary_name"))
        ...
        self.assertDictEqual(
            {
                ...
                "quarantine_location": u"gs://bucket-quarantine/libFuzzer/test_fuzzer/",
...
class CorpusPruningTestFuchsia(unittest.TestCase, BaseTest):
    ...
    def setUp(self):
        ...
        data_types.FuzzTarget(
            binary="example-fuzzers/trap_fuzzer", engine="libFuzzer", project="fuchsia"
        ).put()
    ...
    def test_prune(self):
        ...
        corpus_pruning_task.execute_task(
            "libFuzzer_fuchsia_example-fuzzers-trap_fuzzer", "libfuzzer_asan_fuchsia"
        )
        corpus = os.listdir(self.corpus_dir)
...
class CorpusPruningTestUntrusted(
    ...
    def setUp(self):
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", project="test", binary="test_fuzzer"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="libFuzzer_test_fuzzer",
            engine="libFuzzer",
        ...
        data_types.FuzzTarget(
            engine="libFuzzer", project="test2", binary="fuzzer"
        ).put()
        data_types.FuzzTargetJob(
        ...
        data_types.DataBundle(name="bundle", is_local=True, sync_to_worker=True).put()
        ...
        gsutil.GSUtilRunner().run_gsutil(
            [
                ...
                (corpus_backup_dir + "%s.zip" % corpus_backup_date).format(
    ...
    def test_prune(self):
        ...
        self.mock._record_cross_pollination_stats.side_effect = self.get_mock_record_compare(
        ...
        self.assertDictEqual(
            {
                "corpus_location": u"gs://{}/libFuzzer/test_fuzzer/".format(
                    self.corpus_bucket
                ),
                "corpus_size_bytes": 8,
                "corpus_size_units": 4,
                ...
                "fuzzer": u"test_fuzzer",
                "html_report_url": None,
                "quarantine_location": u"gs://{}/libFuzzer/test_fuzzer/".format(
...
class CrossPollinationTest(unittest.TestCase):
    ...
    def test_select_targets_with_tagged_cross_pollination(self):
        ...
        data_types.CorpusTag(
            tag="test_tag", fully_qualified_fuzz_target_name="libFuzzer_test_fuzzer"
        ).put()

        data_types.CorpusTag(
            tag="test_tag",
            fully_qualified_fuzz_target_name="libFuzzer_cross_pollination_test_fuzzer",
        ).put()
