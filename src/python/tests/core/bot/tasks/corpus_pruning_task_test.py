...
class BaseTest(object):
    ...
    def setUp(self):
        ...
        self.mock.choose_cross_pollination_strategy.return_value = (
            'random', None)
...
class CorpusPruningTest(unittest.TestCase, BaseTest):
    ...
    def test_prune(self):
        ...
        self.assertEqual(
            'label1,label2', testcases[0].get_metadata('issue_labels'))
...
class CorpusPruningTestUntrusted(
    ...
    def setUp(self):
        ...
        self.mock.get_fuzzer_directory.return_value = os.path.join(
            environment.get_value(
                'ROOT_DIR'), 'src', 'python', 'bot', 'fuzzers',
            'libFuzzer')
        ...
        self.corpus = corpus_manager.FuzzTargetCorpus(
            'libFuzzer', 'test_fuzzer')
        self.corpus.rsync_from_disk(
            os.path.join(TEST_DIR, 'corpus'), delete=True)
    ...
    def test_prune(self):
        ...
        self.mock.add_task.assert_has_calls([
            mock.call(
                'minimize', testcases[0].key.id(), u'libfuzzer_asan_job'),
        ])
    ...
    def get_mock_record_compare(self, project_qualified_name, method, sources,
        ...
        def compare(stats):
            ...
            self.assertEqual(project_qualified_name,
                             stats.project_qualified_name)
            ...
            self.assertEqual(initial_edge_coverage,
                             stats.initial_edge_coverage)
            self.assertEqual(edge_coverage, stats.edge_coverage)
            self.assertEqual(initial_feature_coverage,
                             stats.initial_feature_coverage)
...
class CrossPollinationTest(unittest.TestCase):
    ...
    def test_select_targets_with_tagged_cross_pollination(self):
        ...
        data_types.CorpusTag(
            tag='test_tag',
            fully_qualified_fuzz_target_name='libFuzzer_cross_pollination_test_fuzzer').put()
