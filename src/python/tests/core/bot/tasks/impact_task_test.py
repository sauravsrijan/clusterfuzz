...
class ExecuteTaskTest(unittest.TestCase):
    ...
    def test_build_failed_exception(self):
        ...
        self.mock.get_impacts_on_prod_builds.side_effect = impact_task.BuildFailedException(
            "error-from-build"
        )
        impact_task.execute_task(self.testcase.key.id(), "job")
...
class GetImpactsFromUrlTest(ComponentRevisionPatchingTest):
    ...
    def test_bail_out_unknown_component(self):
        ...
        self.assertTrue(impact_task.get_impacts_from_url("123:456", "job").is_empty())
    ...
    def test_bail_out_build_revision_mappings(self):
        ...
        self.mock.get_build_to_revision_mappings.return_value = {}
        self.assertTrue(
            impact_task.get_impacts_from_url("123:456", "job", "windows").is_empty()
        )
        self.mock.get_start_and_end_revision.assert_has_calls(
            [mock.call("123:456", "job")]
        )
...
class GetImpactTest(unittest.TestCase):
    ...
    def test_bail_out_non_digit(self):
        ...
        self.assertTrue(impact_task.get_impact({"revision": "aa"}, 1, 100).is_empty())

    def test_empty_impact(self):
        """Test returning empty impact when start_revision is greater than the build
          revision."""
        self.assertTrue(impact_task.get_impact({"revision": "10"}, 20, 100).is_empty())
...
class GetImpactsOnProdBuilds(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.impacts = impact_task.Impacts(
            stable=impact_task.Impact("s", False), beta=impact_task.Impact("b", True)
        )
    ...
    def test_app_failed_on_stable(self):
        ...
        self.assertEqual(
            self.impacts, impact_task.get_impacts_on_prod_builds(self.testcase, "path")
        )
        self.mock.get_impact_on_build.assert_has_calls(
            [
                mock.call(
                    "stable", self.testcase.impact_stable_version, self.testcase, "path"
                )
            ]
        )
        self.mock.get_impacts_from_url.assert_has_calls(
    ...
    def test_app_failed_on_beta(self):
        ...
        self.mock.get_impact_on_build.assert_has_calls(
            [
                ...
                mock.call(
                    "beta", self.testcase.impact_beta_version, self.testcase, "path"
                ),
            ]
        )
        self.mock.get_impacts_from_url.assert_has_calls([])
...
class GetImpactOnBuild(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock.setup_production_build.return_value = build_manager.ProductionBuild(
            "/base", "52", None, "stable"
        )
    ...
    def test_same_version(self):
        """Test same version."""
        impact = impact_task.get_impact_on_build("stable", "52", self.testcase, "path")
    ...
    def test_crash(self):
        """Test crashing."""
        impact = impact_task.get_impact_on_build("stable", "53", self.testcase, "path")
        self.assertEqual("52", impact.version)
        self.assertFalse(impact.likely)
...
class GetStartAndEndRevisionTest(unittest.TestCase):
    ...
    def test_normal(self):
        ...
        self.mock.get_start_and_end_revision.assert_has_calls([mock.call("123:456")])
    ...
    def test_android(self):
        ...
        self.mock.get_component_range_list.return_value = [
            {"component": "test"},
            {"component": "Chromium", "link_text": "somelink"},
            {"component": "test"},
        ]
        start, end = impact_task.get_start_and_end_revision("123:456", "android_job")
