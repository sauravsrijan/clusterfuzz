...
def create_testcase_list_file(output_directory):
    ...
    files_list = []
    files_list_file_path = os.path.join(
        output_directory, TESTCASE_LIST_FILENAME)
...
def get_resource_dependencies(testcase_absolute_path, test_prefix=FUZZ_PREFIX):
    ...
    if DEPENDENCY_PREFIX in testcase_absolute_path:
        ...
        dependency_file_path = os.path.join(
            base_directory, dependency_filename)
    ...
    if testcase_filename.startswith(test_prefix):
        ...
        resources_filename = '%s%s' % (
            RESOURCES_PREFIX, stripped_testcase_name)
...
class TestcaseRunner(object):
    ...
    def __init__(self,
        ...
        if fuzz_target and engine_impl:
            ...
            arguments = data_handler.filter_arguments(
                arguments, fuzz_target.binary)
...
def get_command_line_for_application(file_to_run='',
    ...
    if file_to_run:
        ...
        relative_testcase_path = file_to_run[len(
            input_directory + os.path.sep):]
    ...
    command = command.replace(
        '%CRASH_STACKTRACES_DIR%', crash_stacks_directory)
...
def setup_user_profile_directory_if_needed(user_profile_directory):
    ...
    if app_name.startswith('firefox'):
        ...
        extensions_directory = os.path.join(
            user_profile_directory, 'extensions')
