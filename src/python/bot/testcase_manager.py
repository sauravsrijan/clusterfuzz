...
CHROME_URL_LOAD_REGEX = re.compile(
    r".*(NetworkDelegate::NotifyBeforeURLRequest|FileURLLoader::Start)" r":\s+(.*)"
)
...
def get_resource_dependencies(testcase_absolute_path, test_prefix=FUZZ_PREFIX):
    ...
    if DEPENDENCY_PREFIX in testcase_absolute_path:
        ...
        dependency_filename = re.compile(FUZZ_PREFIX).sub("", dependency_filename, 1)
        dependency_filename = re.compile(HTTP_PREFIX).sub("", dependency_filename, 1)
...
def get_additional_command_line_flags(testcase_path):
    ...
    flags_file_content = utils.read_data_from_file(flags_file_path, eval_data=False)
...
class Crash(
    collections.namedtuple(
        "Crash",
        "file_path crash_time return_code resource_list gestures " "stack_file_path",
    )
):
        ...
        if platform == "WINDOWS":
            ...
            if platform == "ANDROID":
                ...
                local_path = local_path.replace(
                    remote_testcases_directory, local_testcases_directory
                )
...
def upload_testcase(testcase_path, log_time):
    ...
    fuzzer_logs.upload_to_logs(
        fuzz_logs_bucket, testcase_contents, time=log_time, file_extension=".testcase"
    )
...
def run_testcase_and_return_result_in_queue(
    crash_queue, thread_index, file_path, gestures, env_copy, upload_output=False
):
    """Run a single testcase and return crash results in the crash queue."""
    ...
    logs.configure("run_testcase", {"testcase_path": file_path})

    try:
        # Run testcase and check whether a crash occurred or not.
        return_code, crash_time, output = run_testcase(
            thread_index, file_path, gestures, env_copy
        )
        ...
        if crash_result.is_crash():
            ...
            stack_file_path = os.path.join(
                crash_stacks_directory, utils.string_hash(file_path)
            )
        ...
        if upload_output:
            ...
            log = prepare_log_for_upload(
                crash_result_full.get_stacktrace(), return_code
            )
...
def engine_reproduce(engine_impl, target_name, testcase_path, arguments, timeout):
    ...
    if environment.is_trusted_host():
        ...
        return tasks_host.engine_reproduce(
            engine_impl, target_name, testcase_path, arguments, timeout
        )
    ...
    result = engine_impl.reproduce(target_path, testcase_path, list(arguments), timeout)

    # This matches the check in process_handler.run_process.
    if not result.return_code and (
        crash_analyzer.is_memory_tool_crash(result.output)
        or crash_analyzer.is_check_failure_crash(result.output)
    ):
...
class TestcaseRunner(object):
    ...
    def test_reproduce_reliability(
        self, retries, expected_state, expected_security_flag
    ):
        ...
        for round_number in range(1, retries + 1):
            ...
            if (
                retries - round_number + crash_count + 1
        ...
        logs.log(
            "Crash is not reproducible. Crash count: %d/%d."
...
def test_for_crash_with_retries(
    ...
    try:
        ...
        runner = TestcaseRunner(
            fuzz_target, testcase_path, test_timeout, gestures, http_flag
        )
        ...
        return runner.reproduce_with_retries(
            crash_retries, expected_state, expected_security_flag, testcase.flaky_stack
        )
...
def test_for_reproducibility(
    ...
    try:
        ...
        crash_retries = environment.get_value("CRASH_RETRIES")
        return runner.test_reproduce_reliability(
            crash_retries, expected_state, expected_security_flag
        )
    except TargetNotFoundError:
        # If a target isn't found, treat it as not crashing.
        return False
...
def prepare_log_for_upload(symbolized_output, return_code):
    ...
    component_revisions = (
        revisions.format_revision_list(components, use_html=False) or "Not available.\n"
    )
...
def get_user_profile_directory(user_profile_index):
    ...
    user_profile_root_directory = (
        temp_directory
        if user_profile_in_memory
        else environment.get_value("USER_PROFILE_ROOT_DIR")
    )
    ...
    user_profile_directory = os.path.join(
        user_profile_root_directory, user_profile_directory_name
    )

    return user_profile_directory


def get_command_line_for_application(
    file_to_run="",
    ...
    if file_to_run:
        if launcher:
            ...
            if file_to_run.startswith(local_testcases_directory):
                testcase_relative_path = file_to_run[
                    len(local_testcases_directory) + 1 :
                ]
                testcase_path = os.path.join(
                    device_testcases_directory, testcase_relative_path
                )
                testcase_file_url = utils.file_path_to_file_url(testcase_path)
            else:
    ...
    if (
        not utils.sub_string_exists_in(
            ["%TESTCASE%", "%TESTCASE_FILE_URL%", "%TESTCASE_HTTP_URL%"], all_app_args
        )
        and app_args_append_testcase
    ):
    ...
    command = command.replace("%DEVICE_TESTCASES_DIR%", device_testcases_directory)
    ...
    content_shell_app_names = ["content_shell", "content_shell.exe", "Content Shell"]
    if (
        environment.get_value("APP_NAME") in content_shell_app_names
    ...
    if plt == "ANDROID" and not launcher:
        ...
        return android.app.get_launch_command(
...
def setup_user_profile_directory_if_needed(user_profile_directory):
    ...
    if user_profile_file and ";" in user_profile_file:
        user_profile_filename, encoded_file_contents = user_profile_file.split(";", 1)
        user_profile_file_contents = zlib.decompress(
            base64.b64decode(encoded_file_contents)
        )
        user_profile_file_path = os.path.join(
            user_profile_directory, user_profile_filename
        )
    ...
    if app_name.startswith("firefox"):
        ...
        extension_archive = os.path.join(
            environment.get_resources_directory(), "firefox", "fuzzPriv-extension.zip"
        )
        archive.unpack(extension_archive, extensions_directory)

        # Add this extension in the extensions configuration file.
        extension_config_file_path = os.path.join(
            user_profile_directory, "extensions.ini"
        )
        fuzz_extension_directory = os.path.join(
            extensions_directory, "domfuzz@squarefree.com"
        )
        ...
        utils.write_data_to_file(
            extension_config_file_contents, extension_config_file_path
        )
...
def check_for_bad_build(job_type, crash_revision):
    ...
    return_code, crash_time, output = process_handler.run_process(
        command, timeout=fast_warmup_timeout, current_working_directory=app_directory
    )
    crash_result = CrashResult(return_code, crash_time, output)
    ...
    if (
        crash_result.is_crash(ignore_state=True)
        and not crash_result.should_ignore()
        and not crash_result.get_type() in ["Direct-leak", "Indirect-leak"]
    ):
        is_bad_build = True
    ...
    # then add it now.
    if (
        build_state == data_types.BuildState.UNMARKED
        and not crash_result.should_ignore()
    ):
        data_handler.add_build_metadata(
            job_type, crash_revision, is_bad_build, build_run_console_output
        )
