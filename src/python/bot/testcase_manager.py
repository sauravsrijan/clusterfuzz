...
class Crash(
    collections.namedtuple(
        "Crash",
        "file_path crash_time return_code resource_list gestures "
        "stack_file_path",
    )):
  ...
  if file_to_run:
    if launcher:
      ...
      if file_to_run.startswith(local_testcases_directory):
        testcase_relative_path = file_to_run[len(local_testcases_directory) +
                                             1:]
...
def setup_user_profile_directory_if_needed(user_profile_directory):
  ...
  if user_profile_file and ";" in user_profile_file:
    user_profile_filename, encoded_file_contents = user_profile_file.split(
        ";", 1)
