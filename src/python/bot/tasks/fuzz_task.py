...
SELECTION_METHOD_DISTRIBUTION = [
    SelectionMethod("default", 0.7),
    SelectionMethod("multi_armed_bandit", 0.3),
]
...
class Crash(object):
    ...
    def from_testcase_manager_crash(cls, crash):
        ...
        try:
            orig_unsymbolized_crash_stacktrace = get_unsymbolized_crash_stacktrace(
                crash.stack_file_path
            )
        except Exception:
            logs.log_error(
        ...
        application_command_line = testcase_manager.get_command_line_for_application(
    ...
    def from_engine_crash(cls, crash):
        ...
        )  # TODO(ochang): Write actual command line.
    ...
    def __init__(
        ...
        self.key = "%s,%s,%s" % (self.crash_type, self.crash_state, self.security_flag)
    ...
    def archive_testcase_in_blobstore(self):
        ...
        (
            ...
            self.resource_list, self.file_path
        )
...
class CrashGroup(object):
    ...
    def has_existing_reproducible_testcase(self):
        ...
        return (
            self.existing_testcase and not self.existing_testcase.one_time_crasher_flag
        )
...
class _TrackFuzzTime(object):
    ...
    def __exit__(self, exc_type, value, traceback):
        ...
        monitoring_metrics.FUZZER_TOTAL_FUZZ_TIME.increment_by(
            int(duration), {"fuzzer": self.fuzzer_name, "timeout": self.timeout}
        )
        monitoring_metrics.JOB_TOTAL_FUZZ_TIME.increment_by(
            int(duration), {"job": self.job_type, "timeout": self.timeout}
        )


def _track_fuzzer_run_result(
    fuzzer_name, generated_testcase_count, expected_testcase_count, return_code
):
    """Track fuzzer run result"""
    if expected_testcase_count > 0:
        ratio = float(generated_testcase_count) / expected_testcase_count
        monitoring_metrics.FUZZER_TESTCASE_COUNT_RATIO.add(
            ratio, {"fuzzer": fuzzer_name}
        )
    ...
    monitoring_metrics.FUZZER_RETURN_CODE_COUNT.increment(
        {"fuzzer": fuzzer_name, "return_code": return_code}
    )
    ...
    def sync_from_gcs(self):
        ...
        if last_sync_time:
            ...
            if last_update_time and last_sync_time > last_update_time:
                logs.log(
                    "Corpus for target %s has no new updates, skipping rsync."
                    % self._project_qualified_target_name
                )
                already_synced = True

        time_before_sync_start = time.time()
        result = already_synced or self.gcs_corpus.rsync_to_disk(self._corpus_directory)
        self._synced_files.clear()
        self._synced_files.update(self._walk())

        logs.log(
            "%d corpus files for target %s synced to disk."
            % (len(self._synced_files), self._project_qualified_target_name)
        )
...
def add_additional_testcase_run_data(
...
def read_and_upload_testcase_run_stats(
    fuzzer_name, fully_qualified_fuzzer_name, job_type, revision, testcase_file_paths
):
...
def get_testcases(testcase_count, testcase_directory, data_directory):
    ...
    testcase_directories = get_testcase_directories(testcase_directory, data_directory)
    ...
    generated_testcase_string = "Generated %d/%d testcases." % (
    ...
    if environment.get_value("COMMAND_OVERRIDE") and generated_testcase_count == 0:
        logs.log("No testcases generated. Sleeping for ~30 minutes.")
        time.sleep(random.uniform(1800, 2100))

    return (testcase_file_paths, generated_testcase_count, generated_testcase_string)
...
def pick_timeout_multiplier():
    ...
    custom_timeout_multipliers = environment.get_value("CUSTOM_TIMEOUT_MULTIPLIERS")
    timeout_multiplier = 1.0

    use_multiplier = not utils.random_number(0, DEFAULT_CHOOSE_PROBABILITY)
    if use_multiplier and not fuzz_test_timeout and not custom_timeout_multipliers:
        ...
        # set.
        timeout_multiplier = utils.random_element_from_list(custom_timeout_multipliers)
...
def upload_job_run_stats(
    ...
    _track_testcase_run_result(
        fuzzer_name, job_type, new_crash_count, known_crash_count
    )
...
def store_fuzzer_run_results(
    ...
    save_test_results = (
        not fuzzer.result
        or not fuzzer.result_timestamp
        or dates.time_has_expired(fuzzer.result_timestamp, days=1)
        or (fuzzer_return_code != 0 and fuzzer_return_code != fuzzer.return_code)
        or (
            generated_testcase_count != expected_testcase_count
            and fuzzer.return_code == 0
            and " 0/" not in fuzzer.result
        )
    )
    ...
    truncated_fuzzer_output = truncate_fuzzer_output(
        fuzzer_output, data_types.ENTITY_SIZE_LIMIT
    )
    console_output = u"%s: %s\n%s\n%s" % (
        bot_name,
        fuzzer_return_code_string,
        fuzzer_command,
...
def get_testcase_timeout_multiplier(
    timeout_multiplier, crash, test_timeout, thread_wait_timeout
):
...
def create_testcase(group, context):
    ...
    if (
        crash.file_path in context.testcases_metadata
        and "original_file_path" in context.testcases_metadata[crash.file_path]
        and context.testcases_metadata[crash.file_path]["original_file_path"]
    ):
    ...
    if not (group.one_time_crasher_flag or group.has_existing_reproducible_testcase()):
        crash.crash_info = crash_uploader.save_crash_info_if_needed(
            testcase_id,
            context.crash_revision,
            context.job_type,
            crash.crash_type,
            crash.crash_address,
            crash.crash_frames,
...
def filter_crashes(crashes):
    ...
    for crash in crashes:
        if not crash.is_valid():
            logs.log(
                "Ignore crash (reason=%s, type=%s, state=%s)."
...
def write_crashes_to_big_query(group, context):
    ...
    insert_id_prefix = ":".join(
        [group.crashes[0].key, context.bot_name, str(created_at)]
    )
...
def _update_testcase_variant_if_needed(group, context):
    ...
    variant = data_handler.get_testcase_variant(
...
def process_crashes(crashes, context):
    ...
    for _, grouped_crashes in group_of_crashes:
        ...
        if not group.main_crash:
            logs.log(
                "Unable to store testcase in blobstore: %s"
        ...
        logs.log(
            ...
            "crash stacktrace=%s)"
            % (
                group.main_crash.filename,
                group.main_crash.fuzzed_key,
                group.main_crash.return_code,
                group.main_crash.crash_time,
                group.main_crash.crash_type,
                group.main_crash.crash_state,
                group.main_crash.security_flag,
                group.main_crash.crash_stacktrace,
            )
        )

        if group.should_create_testcase():
            group.newly_created_testcase = create_testcase(group=group, context=context)
        else:
    ...
    logs.log(
        "New crashes: {}, known crashes: {}, processed groups: {}".format(
...
def run_engine_fuzzer(
    ...
    fuzz_test_timeout = environment.get_value("FUZZ_TEST_TIMEOUT")
    result = engine_impl.fuzz(
        target_path, options, testcase_directory, fuzz_test_timeout
    )
    ...
    log_header = engine_common.get_log_header(
        result.command, environment.get_value("BOT_NAME"), result.time_executed
    )

    formatted_strategies = engine_common.format_fuzzing_strategies(options.strategies)

    result.logs = log_header + "\n" + result.logs + "\n" + formatted_strategies

    fuzzer_metadata = {"fuzzer_binary_name": target_name}
...
class FuzzingSession(object):
    ...
    def sync_new_corpus_files(self):
        ...
        logs.log(
            "%d new corpus files generated by fuzzer %s (job %s)."
            % (
                new_files_count,
                self.fuzz_target.project_qualified_name(),
                self.job_type,
        ...
        if new_files_count > MAX_NEW_CORPUS_FILES:
            ...
            logs.log(
                (
                    "Only uploading %d out of %d new corpus files "
                    "generated by fuzzer %s (job %s)."
                )
                % (
                    MAX_NEW_CORPUS_FILES,
                    new_files_count,
                    self.fuzz_target.project_qualified_name(),
                    self.job_type,
                )
    ...
    def generate_blackbox_testcases(self, fuzzer, fuzzer_directory, testcase_count):
        ...
        testcase_directories = get_testcase_directories(
            self.testcase_directory, self.data_directory
        )
        ...
        if fuzzer.builtin:
            ...
            fuzzer_return_code, fuzzer_duration, fuzzer_output = process_handler.run_process(
                fuzzer_command,
        ...
        testcase_file_paths, generated_testcase_count, generated_testcase_string = get_testcases(
            testcase_count, self.testcase_directory, self.data_directory
        )

        # Check for process return code to identify abnormal termination.
        if fuzzer_return_code:
            if (
                float(generated_testcase_count) / testcase_count
                < FUZZER_FAILURE_THRESHOLD
            ):
                logs.log_error(
        ...
        _track_fuzzer_run_result(
            fuzzer_name, generated_testcase_count, testcase_count, fuzzer_return_code
        )
    ...
    def do_engine_fuzzing(self, engine_impl):
        ...
        self.fuzz_target = record_fuzz_target(
    ...
    def do_blackbox_fuzzing(self, fuzzer, fuzzer_directory, job_type):
        ...
        if fuzz_test_timeout:
            test_timeout = set_test_timeout(fuzz_test_timeout, self.timeout_multiplier)
        else:
        ...
        (
            ...
            sync_corpus_directory,
            fuzzer_metadata,
        ) = self.generate_blackbox_testcases(fuzzer, fuzzer_directory, testcase_count)
        ...
        if environment.get_value("USE_BANDIT_STRATEGY_SELECTION"):
            ...
            environment.set_value(
                "STRATEGY_SELECTION_METHOD", selection_method.method_name
            )
        ...
        while test_number < len(testcase_file_paths):
            ...
            with _TrackFuzzTime(self.fully_qualified_fuzzer_name, job_type) as tracker:
    ...
    def run(self):
        ...
        fuzzer = data_types.Fuzzer.query(
            data_types.Fuzzer.name == self.fuzzer_name
        ).get()
        if not fuzzer or not setup.update_fuzzer_and_data_bundles(self.fuzzer_name):
            _track_fuzzer_run_result(
                self.fuzzer_name, 0, 0, FuzzErrorCode.FUZZER_SETUP_FAILED
            )
        ...
        if not build_manager.check_app_path():
            _track_fuzzer_run_result(
                self.fuzzer_name, 0, 0, FuzzErrorCode.BUILD_SETUP_FAILED
            )
            return
        ...
        is_bad_build = testcase_manager.check_for_bad_build(
        ...
        if not self.data_directory:
            _track_fuzzer_run_result(
                self.fuzzer_name, 0, 0, FuzzErrorCode.DATA_BUNDLE_SETUP_FAILED
            )
            logs.log_error("Unable to setup data bundle %s." % fuzzer.data_bundle_name)
            return
        ...
        if engine_impl:
            ...
            fuzzer_metadata, testcase_file_paths, testcases_metadata, crashes = self.do_blackbox_fuzzing(
