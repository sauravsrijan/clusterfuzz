...
class Crash(object):
    ...
    def from_engine_crash(cls, crash, fuzzing_strategies):
        ...
        return Crash(
            ...
            resource_list=[],
            gestures=[],
            unsymbolized_crash_stacktrace=utils.decode_to_unicode(
                crash.stacktrace),
    ...
    def get_error(self):
        ...
        filter_functional_bugs = environment.get_value(
            'FILTER_FUNCTIONAL_BUGS')
...
class GcsCorpus(object):
    ...
    def sync_from_gcs(self):
        ...
        if environment.is_trusted_host():
            from bot.untrusted_runner import file_host
            worker_sync_file_path = file_host.rebase_to_worker_root(
                sync_file_path)
            shell.remove_file(sync_file_path)
            file_host.copy_file_from_worker(
                worker_sync_file_path, sync_file_path)
        ...
        if last_sync_time and os.path.exists(self._corpus_directory):
            last_update_time = storage.last_updated(
                self.gcs_corpus.get_gcs_url())
        ...
        if result and self._synced_files and not already_synced:
            ...
            if environment.is_trusted_host():
                ...
                file_host.copy_file_to_worker(
                    sync_file_path, worker_sync_file_path)
...
def _add_issue_metadata_from_environment(metadata):
    ...
    if labels:
        metadata['issue_labels'] = _append(
            metadata.get('issue_labels'), labels)
...
def run_engine_fuzzer(engine_impl, target_name, sync_corpus_directory,
    ...
    options = engine_impl.prepare(
        sync_corpus_directory, target_path, build_dir)
...
class FuzzingSession(object):
    ...
    def __init__(self, fuzzer_name, job_type, test_timeout):
        ...
        self.test_timeout = set_test_timeout(
            test_timeout, self.timeout_multiplier)
    ...
    def _save_fuzz_targets_count(self):
        ...
        targets_count = ndb.Key(
            data_types.FuzzTargetsCount, self.job_type).get()
    ...
    def generate_blackbox_testcases(self, fuzzer, fuzzer_directory,
        ...
        testcase_directories = get_testcase_directories(self.testcase_directory,
                                                        self.data_directory)
        testcase_manager.remove_testcases_from_directories(
            testcase_directories)
        ...
        if fuzzer.builtin:
            ...
            fuzzer_executable = os.path.join(
                fuzzer_directory, fuzzer.executable_path)
            ...
            command_format = (
                '%s --input_dir%s%s --output_dir%s%s --no_of_files%s%d')
            fuzzer_command = str(
    ...
    def do_blackbox_fuzzing(self, fuzzer, fuzzer_directory, job_type):
        ...
        if environment.get_value('USE_BANDIT_STRATEGY_SELECTION'):
            ...
            if distribution:
                environment.set_value(
                    'STRATEGY_SELECTION_DISTRIBUTION', distribution)
        ...
        while test_number < len(testcase_file_paths):
            ...
            with _TrackFuzzTime(self.fully_qualified_fuzzer_name,
                                job_type) as tracker:
                tracker.timeout = utils.wait_until_timeout(
                    threads, thread_timeout)
    ...
    def run(self):
        ...
        dataflow_bucket_path = environment.get_value(
