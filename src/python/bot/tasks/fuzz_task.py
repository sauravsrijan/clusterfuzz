...
class Crash(object):
  ...
  def from_testcase_manager_crash(cls, crash):
    ...
    try:
      ...
      logs.log_error("Unable to read stacktrace from file %s." %
                     crash.stack_file_path)
      return None
...
class GcsCorpus(object):
  ...
  def sync_from_gcs(self):
    ...
    logs.log("%d corpus files for target %s synced to disk." %
...
def get_testcases(testcase_count, testcase_directory, data_directory):
  ...
  if environment.get_value(
      "COMMAND_OVERRIDE") and generated_testcase_count == 0:
...
def process_crashes(crashes, context):
  ...
  for _, grouped_crashes in group_of_crashes:
    ...
    logs.log("Process the crash group (file=%s, "
             "fuzzed_key=%s, "
             "return code=%s, "
             "crash time=%d, "
             "crash type=%s, "
...
class FuzzingSession(object):
  ...
  def generate_blackbox_testcases(self, fuzzer, fuzzer_directory,
    ...
    if fuzzer.builtin:
      ...
      builtin_result = builtin_fuzzer.run(self.data_directory,
      ...
      if not fuzzer.executable_path:
        logs.log_error("Fuzzer %s does not have an executable path." %
                       fuzzer_name)
        error_occurred = True
      ...
      fuzzer_command = str(command_format % (
          command,
          argument_seperator,
          self.data_directory,
          argument_seperator,
    ...
    if fuzzer_return_code:
      if (float(generated_testcase_count) / testcase_count <
  ...
  def do_blackbox_fuzzing(self, fuzzer, fuzzer_directory, job_type):
    ...
    (
        ...
        sync_corpus_directory,
        fuzzer_metadata,
    ) = self.generate_blackbox_testcases(fuzzer, fuzzer_directory,
                                         testcase_count)
  ...
  def run(self):
    ...
    if (build_manager.setup_build(
      ...
      build_manager.setup_trunk_build([dataflow_bucket_path],
    ...
    if not self.data_directory:
      ...
      logs.log_error("Unable to setup data bundle %s." %
                     fuzzer.data_bundle_name)
      return
