...
class FuzzingSession(object):
    ...
    def sync_new_corpus_files(self):
        ...
        if filtered_new_files_count < new_files_count:
            logs.log(
                (
                    "Uploading only %d out of %d new corpus files "
                    "generated by fuzzer %s (job %s)."
                )
                % (
                    filtered_new_files_count,
                    new_files_count,
                    self.fuzz_target.project_qualified_name(),
                    self.job_type,
                )
    ...
    def generate_blackbox_testcases(self, fuzzer, fuzzer_directory, testcase_count):
        ...
        (
            ...
            generated_testcase_count,
            generated_testcase_string,
        ) = get_testcases(testcase_count, self.testcase_directory, self.data_directory)
            ...
            if (
                ...
                logs.log_error(
                    (
                        ...
                        "(fuzzer={name}, return_code={return_code})."
