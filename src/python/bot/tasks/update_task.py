...
def _platform_deployment_filename():
    ...
    platform_mappings = {"Linux": "linux", "Windows": "windows", "Darwin": "macos"}
...
def get_newer_source_revision():
    ...
    if environment.get_value("LOCAL_SRC") or environment.get_value("LOCAL_DEVELOPMENT"):
    ...
    if (
        not get_source_url()
        or not source_manifest_url
        or not temp_directory
        or not root_directory
    ):
...
def update_source_code():
    ...
    for filepath in file_list:
        ...
        try:
            ...
            logs.log_error(
                "Failed to remove or move %s before extracting new "
        ...
        try:
            ...
            error_occurred = True
            logs.log_error("Failed to extract file %s from source archive." % filepath)
    ...
    local_manifest_path = os.path.join(root_directory, utils.LOCAL_SOURCE_MANIFEST)
    source_version = (
        utils.read_data_from_file(local_manifest_path, eval_data=False)
        .decode("utf-8")
        .strip()
    )
...
def update_tests_if_needed():
    ...
    tasks.track_task_start(tasks.Task("update_tests", "", ""), expected_task_duration)
