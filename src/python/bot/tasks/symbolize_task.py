...
def execute_task(testcase_id, job_type):
    ...
    if environment.tool_matches("ASAN", job_type) and testcase.security_flag:
        ...
        while redzone >= MIN_REDZONE:
            ...
            if crash_result.is_crash() and "AddressSanitizer" in output:
                ...
                if (
                    not crash_analyzer.ignore_stacktrace(state.crash_stacktrace)
                    and security_flag == testcase.security_flag
                    and state.crash_type == testcase.crash_type
                    and (
                        state.crash_type != sym_crash_type
    ...
    if not symbolized_builds or (
        not build_manager.check_app_path()
    ...
    testcase.crash_stacktrace = data_handler.filter_stacktrace(sym_crash_stacktrace)

    if not result:
        data_handler.update_testcase_comment(
            testcase,
            data_types.TaskState.ERROR,
            "Unable to reproduce crash, skipping " "stacktrace update",
        )
    else:
...
def get_symbolized_stacktraces(
    testcase_file_path, testcase, old_crash_stacktrace, expected_state
):
