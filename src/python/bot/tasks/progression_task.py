...
def _update_completion_metadata(testcase, revision, is_crash=False, message=None):
...
def _check_fixed_for_custom_binary(testcase, job_type, testcase_file_path):
    ...
    if not build_manager.check_app_path():
        testcase = data_handler.get_testcase_by_id(testcase_id)
        data_handler.update_testcase_comment(
            testcase, data_types.TaskState.ERROR, "Build setup failed for custom binary"
        )
...
def _update_issue_metadata(testcase):
    ...
    for key, value in six.iteritems(metadata):
        ...
        if old_value != value:
            logs.log(
                "Updating issue metadata for {} from {} to {}.".format(
                    key, old_value, value
                )
            )
...
def _testcase_reproduces_in_revision(
    testcase, testcase_file_path, job_type, revision, update_metadata=False
):
...
def _save_current_fixed_range_indices(testcase_id, fixed_range_start, fixed_range_end):
...
def find_fixed_range(testcase_id, job_type):
    ...
    if not revision_list:
        testcase = data_handler.get_testcase_by_id(testcase_id)
        data_handler.update_testcase_comment(
            testcase, data_types.TaskState.ERROR, "Failed to fetch revision list"
        )
        tasks.add_task("progression", testcase_id, job_type)
        return
    ...
    testcase = data_handler.get_testcase_by_id(testcase_id)
    data_handler.update_testcase_comment(
        testcase, data_types.TaskState.STARTED, "r%d" % max_revision
    )

    # Check to see if this testcase is still crashing now. If it is, then just
    # bail out.
    result = _testcase_reproduces_in_revision(
        testcase, testcase_file_path, job_type, max_revision, update_metadata=True
    )
    if result.is_crash():
        logs.log(
            "Found crash with same signature on latest revision r%d." % max_revision
        )
        app_path = environment.get_value("APP_PATH")
        command = testcase_manager.get_command_line_for_application(
            testcase_file_path, app_path=app_path, needs_http=testcase.http_flag
    ...
    if result and not result.is_crash():
        ...
        _clear_progression_pending(testcase)
        error_message = "Known crash revision %d did not crash" % known_crash_revision
        data_handler.update_testcase_comment(
            testcase, data_types.TaskState.ERROR, error_message
        )
    ...
    while time.time() < deadline:
        ...
        # cases with flaky stacks, but the exact cause is unknown.
            ...
            testcase.open = False
            message = (
                "Fixed testing errored out (min and max revisions "
