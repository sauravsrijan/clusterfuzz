...
def _copy_testcase_to_device_and_setup_environment(testcase,
  ...
  if testcase_file_path and testcase_file_path.startswith(
...
def _get_testcase_file_and_path(testcase):
  ...
  testcase_path_is_absolute = testcase_absolute_path[
...
def update_fuzzer_and_data_bundles(fuzzer_name):
  ...
  if not fuzzer.builtin and revisions.needs_update(version_file,
                                                   fuzzer.revision):
