...
def setup_testcase(testcase, job_type, fuzzer_override=None):
    ...
    if environment.platform() == 'ANDROID':
        _copy_testcase_to_device_and_setup_environment(
    ...
    if archived:
        if minimized:
            ...
            temp_filename = os.path.join(
                input_directory, testcase.archive_filename)
    else:
...
def archive_testcase_and_dependencies_in_gcs(resource_list, testcase_path):
    ...
    if len(resource_list) <= 1:
        ...
        zip_path = os.path.join(
            environment.get_value('INPUT_DIR'), zip_filename)
