...
def _set_timeout_value_from_user_upload(testcase_id):
    ...
    metadata = data_types.TestcaseUploadMetadata.query(
        data_types.TestcaseUploadMetadata.testcase_id == int(testcase_id)
    ).get()
    if metadata and metadata.timeout:
        environment.set_value("TEST_TIMEOUT", metadata.timeout)


def _copy_testcase_to_device_and_setup_environment(testcase, testcase_file_path):
    ...
    if testcase_file_path and testcase_file_path.startswith(local_testcases_directory):
        relative_testcase_file_path = testcase_file_path[
...
def prepare_environment_for_testcase(testcase, job_type, task_name):
    ...
    if testcase.timeout_multiplier:
        ...
        environment.set_value(
            "TEST_TIMEOUT", int(test_timeout * testcase.timeout_multiplier)
        )
...
def setup_testcase(testcase, job_type, fuzzer_override=None):
    ...
    if fuzzer_name:
        try:
            ...
            logs.log_error(
                "Closed testcase %d with invalid fuzzer %s."
...
def _get_testcase_file_and_path(testcase):
    ...
    testcase_path_is_absolute = testcase_absolute_path[1:3] == ":\\" or os.path.isabs(
        testcase_absolute_path
    )
    ...
    if archived:
        if minimized:
            temp_filename = os.path.join(
                input_directory, str(testcase.key.id()) + _TESTCASE_ARCHIVE_EXTENSION
            )
        else:
...
def _clear_old_data_bundles_if_needed():
    ...
    dirs_to_remove = sorted(dirs, key=os.path.getmtime, reverse=True)[
        _DATA_BUNDLE_CACHE_COUNT:
    ]
...
def update_data_bundle(fuzzer, data_bundle):
    ...
    if not shell.create_directory(data_bundle_directory, create_intermediates=True):
        logs.log_error("Failed to create data bundle %s directory." % data_bundle.name)
        return False
    ...
    if not _is_search_index_data_bundle(data_bundle.name):
        ...
        if result.return_code != 0:
            logs.log_error(
                "Failed to sync data bundle %s: %s." % (data_bundle.name, result.output)
            )
            _release_lock_for_data_bundle_update(data_bundle)
            return False
...
def update_fuzzer_and_data_bundles(fuzzer_name):
    ...
    if not fuzzer.builtin and revisions.needs_update(version_file, fuzzer.revision):
        ...
        if not os.path.exists(fuzzer_path):
            error_message = (
                "Fuzzer executable %s not found. " "Check fuzzer configuration."
            ) % fuzzer.executable_path
            logs.log_error(error_message)
            fuzzer_logs.upload_script_log(
                "Fatal error: " + error_message, fuzzer_name=fuzzer_name
            )
    ...
    if fuzzer.launcher_script:
        fuzzer_launcher_path = os.path.join(fuzzer_directory, fuzzer.launcher_script)
        ...
        if environment.is_trusted_host():
            ...
            worker_fuzzer_directory = file_host.rebase_to_worker_root(fuzzer_directory)
...
def _is_data_bundle_up_to_date(data_bundle, data_bundle_directory):
    ...
    if last_updated_time and last_sync_time > last_updated_time:
        logs.log("Data bundle %s has no new content from last sync." % data_bundle.name)
        return True
...
def _get_nfs_data_bundle_path(data_bundle_name):
    ...
    if _is_search_index_data_bundle(data_bundle_name):
        return os.path.join(
            nfs_root,
            testcase_manager.SEARCH_INDEX_TESTCASES_DIRNAME,
...
def get_data_bundle_directory(fuzzer_name):
    ...
    local_data_bundle_directory = os.path.join(
        local_data_bundles_directory, data_bundle.name
    )
    ...
    if len(resource_list) <= 1:
        ...
        for list_index in range(1, len(resource_list)):
            ...
            for directory_index in range(length):
                if (
                    current_directory_list[directory_index]
