...
from crash_analysis import crash_analyzer, severity_analyzer
...
def execute_task(testcase_id, job_type):
    ...
    if not metadata:
        logs.log_error("Testcase %s has no associated upload metadata." % testcase_id)
        testcase.key.delete()
        return
    ...
    if not build_manager.check_app_path():
        data_handler.update_testcase_comment(
            testcase, data_types.TaskState.ERROR, "Build setup failed"
        )

        if data_handler.is_first_retry_for_task(testcase):
            build_fail_wait = environment.get_value("FAIL_WAIT")
            tasks.add_task("analyze", testcase_id, job_type, wait_time=build_fail_wait)
        else:
            close_invalid_testcase_and_update_status(
    ...
    if not crashed:
        ...
        close_invalid_testcase_and_update_status(testcase, metadata, "Unreproducible")
    ...
    crash_stacktrace_output = utils.get_crash_stacktrace_output(
        application_command_line, state.crash_stacktrace, unsymbolized_crash_stacktrace
    )
    testcase.crash_stacktrace = data_handler.filter_stacktrace(crash_stacktrace_output)
