...
def update_environment_for_job(environment_string):
    ...
    environment_values = environment.parse_environment_definition(environment_string)
    ...
    fuzz_test_timeout_override = environment.get_value("FUZZ_TEST_TIMEOUT_OVERRIDE")
...
def run_command(task_name, task_argument, job_name):
    ...
    if should_update_task_status(task_name):
        if not data_handler.update_task_status(
            task_state_name, data_types.TaskState.STARTED
        ):
            logs.log(
                'Another instance of "{}" already '
    ...
    try:
        ...
        if should_update_task_status(task_name):
            data_handler.update_task_status(task_state_name, data_types.TaskState.ERROR)

        raise

    # Task completed successfully.
    if should_update_task_status(task_name):
        data_handler.update_task_status(task_state_name, data_types.TaskState.FINISHED)
...
def process_command(task):
    ...
    if job_name != "none":
        ...
        if testcase:
            ...
            if (
                task_name != "variant"
                and testcase_platform_id
                and not utils.fields_match(testcase_platform_id, current_platform_id)
            ):
                logs.log(
        ...
        if task_name == "minimize":
            ...
            if minimize_job_override:
                minimize_job = data_types.Job.query(
                    data_types.Job.name == minimize_job_override
                ).get()
                if minimize_job:
        ...
        if fuzzer_name:
            ...
            if (
                fuzzer
                and hasattr(fuzzer, "additional_environment_string")
    ...
    if not is_supported_cpu_arch_for_job():
        logs.log("Unsupported cpu architecture specified in job definition, exiting.")
        tasks.add_task(task_name, task_argument, job_name)
        return
