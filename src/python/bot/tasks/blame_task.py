...
def _format_component_revisions_for_predator(component_revisions):
    ...
    for path, entry in six.iteritems(component_revisions):
        ...
        result.append(
            {"dep_path": path, "repo_url": url, "revision": sha,}
        )
...
def _set_predator_result_with_error(testcase, error_message):
    ...
    testcase.set_metadata("predator_result", predator_result, update_testcase=False)
...
def _prepare_predator_message(testcase):
    ...
    if ":" in testcase.regression:
        ...
        start_revisions_dict, start_revision_hash = _prepare_component_revisions_dict(
            start_revision, testcase.job_type
        )
        # Do a None check since we can return {} for revision = 0.
        if start_revisions_dict is None:
            _set_predator_result_with_error(
                testcase,
                "Failed to fetch component revisions for revision %s." % start_revision,
            )
            return None
        ...
        if start_revision != 0:
            dependency_rolls = _compute_rolls(start_revisions_dict, end_revisions_dict)

    # Put the current revisions dictionary in the format predator expects.
    crash_revision_component_revisions_list = _format_component_revisions_for_predator(
    ...
    src_entry = [
        entry
        for entry in crash_revision_component_revisions_list
