...
class MinijailLibFuzzerRunner(
  ...
  def _get_chroot_directory(self, directory_path):
    ...
    if not binding:
      raise LibFuzzerException('Failed to get chroot binding for "%s".' %
                               directory_path)
...
def pick_strategies(strategy_pool,
  ...
  if use_dataflow_tracing:
    ...
    if os.path.exists(dataflow_trace_dir):
      arguments.append("%s%s" %
                       (constants.DATA_FLOW_TRACE_FLAG, dataflow_trace_dir))
  ...
  if strategy_pool.do_strategy(
      strategy.RECOMMENDED_DICTIONARY_STRATEGY) and add_recommended_dictionary(
  ...
  if (not is_fuchsia and not is_android and not is_ephemeral and
    ...
    fuzzing_strategies.append("%s_%d" %
                              (strategy.FORK_STRATEGY.name, num_fuzz_processes))

  extra_env = {}
  if strategy_pool.do_strategy(
      strategy.MUTATOR_PLUGIN_STRATEGY) and use_mutator_plugin(
          target_name, extra_env):
