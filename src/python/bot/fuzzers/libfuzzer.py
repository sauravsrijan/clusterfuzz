...
CRASH_TESTCASE_REGEX = r".*Test unit written to\s*" r"(.*(crash|oom|timeout|leak)-.*)"
...
class LibFuzzerCommon(object):
    ...
    def run_single_testcase(self, testcase_path, timeout=None, additional_args=None):
...
class UnshareLibFuzzerRunner(
    new_process.UnshareProcessRunnerMixin,
    new_process.UnicodeProcessRunner,
    LibFuzzerCommon,
):
    """LibFuzzerRunner which unshares."""


class FuchsiaQemuLibFuzzerRunner(new_process.UnicodeProcessRunner, LibFuzzerCommon):
    ...
    def _setup_device_and_fuzzer(self):
        ...
        test_data_dir = os.path.join(
            fuchsia_resources_dir_plus_build,
            self.FUZZER_TEST_DATA_REL_PATH,
            package,
            target,
        )
    ...
    def process_logs_and_crash(self, artifact_prefix):
        ...
        with open(processed_log_path, mode="w", encoding="utf-8") as new_file:
            with open(
                self.fuzzer.logfile, encoding="utf-8", errors="ignore"
            ) as old_file:
                for line in old_file:
                    ...
                    if line_match:
                        ...
                        line = re.sub(
                            crash_location_regex,
                            r"\1\2" + crash_testcase_file_path,
                            line,
                        )
                    new_file.write(line)
    ...
    def _test_qemu_ssh(self):
        ...
        ssh_test_process = new_process.ProcessRunner(
            "ssh",
            self.device.get_ssh_cmd(["ssh", "localhost", "echo running on fuchsia!"])[
                1:
            ],
        )
    ...
    def _pull_new_corpus_from_target_to_host(self, corpus_directories, artifact_prefix):
        ...
        files_in_new_corpus_dir_target = (
            self._new_corpus_dir_target(corpus_directories) + "/*"
        )
        self.fuzzer.device.fetch(
            files_in_new_corpus_dir_target,
    ...
    def fuzz(
        ...
        return_code = self.fuzzer.start(
            self._corpus_directories_libfuzzer(corpus_directories) + additional_args
        )
        self.fuzzer.monitor(return_code)
        self.process_logs_and_crash(artifact_prefix)
        with open(self.fuzzer.logfile, encoding="utf-8", errors="ignore") as logfile:
            symbolized_output = logfile.read()

        self._pull_new_corpus_from_target_to_host(corpus_directories, artifact_prefix)
        self._clear_all_target_corpora()
...
class MinijailLibFuzzerRunner(
    new_process.UnicodeProcessRunnerMixin,
    ...
    def __init__(self, executable_path, chroot, default_args=None):
        ...
        super(MinijailLibFuzzerRunner, self).__init__(
            executable_path=executable_path, chroot=chroot, default_args=default_args
        )
    ...
    def get_testcase_path(self, log_lines):
        ...
        for binding in self.chroot.bindings:
            if path.startswith(binding.dest_path):
                return os.path.join(
                    binding.src_path, os.path.relpath(path, binding.dest_path)
                )
    ...
    def run_single_testcase(self, testcase_path, timeout=None, additional_args=None):
        ...
        with self._chroot_testcase(testcase_path) as chroot_testcase_path:
            return LibFuzzerCommon.run_single_testcase(
                self, chroot_testcase_path, timeout, additional_args
            )
...
class AndroidLibFuzzerRunner(new_process.UnicodeProcessRunner, LibFuzzerCommon):
    ...
    def _copy_local_directory_to_device(self, local_directory):
        ...
        android.adb.copy_local_directory_to_remote(local_directory, device_directory)
    ...
    def _copy_local_directories_from_device(self, local_directories):
        ...
        for local_directory in sorted(set(local_directories)):
            ...
            android.adb.copy_remote_directory_to_local(
                device_directory, local_directory
            )
    ...
    def fuzz(
        ...
        if additional_args is None:
            additional_args = []
        dict_path = fuzzer_utils.extract_argument(additional_args, constants.DICT_FLAG)
        if dict_path:
    ...
    def run_single_testcase(self, testcase_path, timeout=None, additional_args=None):
        ...
        with self._device_file(testcase_path) as device_testcase_path:
            return LibFuzzerCommon.run_single_testcase(
                self, device_testcase_path, timeout, additional_args
            )
...
def get_runner(fuzzer_path, temp_dir=None, use_minijail=None, use_unshare=None):
    ...
    if use_minijail or is_chromeos_system_job:
        ...
        llvm_symbolizer_destination_path = os.path.join(minijail_bin, "llvm-symbolizer")
...
def add_recommended_dictionary(arguments, fuzzer_name, fuzzer_path):
    ...
    recommended_dictionary_path = os.path.join(
        fuzzer_utils.get_temp_dir(), dictionary_manager.RECOMMENDED_DICTIONARY_FILENAME
    )
...
def get_dictionary_analysis_timeout():
    ...
    return engine_common.get_overridable_timeout(5 * 60, "DICTIONARY_TIMEOUT_OVERRIDE")


def analyze_and_update_recommended_dictionary(
    runner, fuzzer_name, log_lines, corpus_directory, arguments
):
    ...
    logs.log("Extracting and analyzing recommended dictionary for %s." % fuzzer_name)

    # Extract recommended dictionary elements from the log.
    dict_manager = dictionary_manager.DictionaryManager(fuzzer_name)
    recommended_dictionary = dict_manager.parse_recommended_dictionary_from_log_lines(
        log_lines
    )
    ...
    temp_dictionary_path = os.path.join(
        fuzzer_utils.get_temp_dir(), temp_dictionary_filename
    )
    ...
    if dictionary_analysis.timed_out:
        logs.log_warn("Recommended dictionary analysis for %s timed out." % fuzzer_name)
        return None

    if dictionary_analysis.return_code != 0:
        logs.log_warn(
            "Recommended dictionary analysis for %s failed: %d."
    ...
    logs.log(
        "Added %d new elements to the recommended dictionary for %s."
...
def set_sanitizer_options(fuzzer_path, fuzz_options=None):
    ...
    sanitizer_options = environment.get_memory_tool_options(sanitizer_options_var, {})
...
def is_linux_asan():
    ...
    return (
        ...
        or environment.get_value("MEMORY_TOOL") != "ASAN"
    )
...
def use_radamsa_mutator_plugin(extra_env):
    ...
    radamsa_path = os.path.join(
        environment.get_platform_resources_directory(), "radamsa", "libradamsa.so"
    )
...
def use_peach_mutator(extra_env, grammar):
    ...
    peach_dir = os.path.join(environment.get_platform_resources_directory(), "peach")
...
def pick_strategies(
    strategy_pool, fuzzer_path, corpus_directory, existing_arguments, grammar=None
):
    """Pick strategies."""
    ...
    is_mutations_run = (
        not environment.is_ephemeral()
        and candidate_generator != engine_common.Generator.NONE
    )
    ...
    if (
        ...
        and not use_dataflow_tracing
        and strategy_pool.do_strategy(strategy.FORK_STRATEGY)
    ):
    ...
    ) and use_mutator_plugin(target_name, extra_env):
        fuzzing_strategies.append(strategy.MUTATOR_PLUGIN_STRATEGY.name)

    if (
        not has_existing_mutator_strategy(fuzzing_strategies)
        and strategy_pool.do_strategy(strategy.PEACH_GRAMMAR_MUTATION_STRATEGY)
        and use_peach_mutator(extra_env, grammar)
    ):
        fuzzing_strategies.append(
    ...
    if (
        not has_existing_mutator_strategy(fuzzing_strategies)
        and strategy_pool.do_strategy(strategy.MUTATOR_PLUGIN_RADAMSA_STRATEGY)
        and use_radamsa_mutator_plugin(extra_env)
    ):
