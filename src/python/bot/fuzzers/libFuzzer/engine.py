...
from bot.fuzzers import (dictionary_manager, engine, engine_common, libfuzzer,
                         strategy_selection)
from bot.fuzzers import utils as fuzzer_utils
from bot.fuzzers.libFuzzer import constants, fuzzer, stats
...
def _is_multistep_merge_supported(target_path):
    ...
    if os.path.exists(target_path):
        with open(target_path, "rb") as file_handle:
            return utils.search_bytes_in_file(
                MULTISTEP_MERGE_SUPPORT_TOKEN, file_handle
            )
...
class LibFuzzerEngine(engine.Engine):
    ...
    def prepare(self, corpus_dir, target_path, _):
        ...
        strategy_info = libfuzzer.pick_strategies(
            strategy_pool, target_path, corpus_dir, arguments, grammar
        )
        ...
        if (
            not strategy_info.use_dataflow_tracing
    ...
    def fuzz(self, target_path, options, reproducers_dir, max_time):
        ...
        if dict_error_match:
            logs.log_error(
                "Dictionary parsing failed (target={target}, line={line}).".format(
                    target=project_qualified_fuzzer_name, line=dict_error_match.group(1)
                ),
                engine_output=fuzz_result.output,
            )
        elif (
            not environment.get_value("USE_MINIJAIL")
        ...
        if not crash_testcase_file_path and fuzz_result.return_code:
            crash_testcase_file_path = self._create_empty_testcase_file(reproducers_dir)
        ...
        parsed_stats.update(
            stats.parse_performance_features(
                log_lines, options.strategies, options.arguments
            )
        )
    ...
    def _minimize_corpus_two_step(
        ...
        merge_stats["new_features"] = (
            merge_stats["feature_coverage"] - merge_stats["initial_feature_coverage"]
        )
    ...
    def minimize_corpus(
        ...
        # TODO(ochang): Get crashes found during merge.
        return engine.FuzzResult(
            merge_output, result.command, [], merge_stats, result.time_executed
        )
