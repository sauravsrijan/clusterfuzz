...
def generate_new_testcase_mutations(
    corpus_directory, new_testcase_mutations_directory, fuzzer_name, candidate_generator
):
...
def generate_new_testcase_mutations_using_radamsa(
    ...
    filtered_files_list = [
        f for f in files_list if os.path.getsize(f) <= RADAMSA_INPUT_FILE_SIZE_LIMIT
    ]
    if not filtered_files_list:
        # No mutations to do on an empty corpus or one with very large files.
        return

    old_corpus_size = shell.get_directory_file_count(new_testcase_mutations_directory)
    ...
    new_corpus_size = shell.get_directory_file_count(new_testcase_mutations_directory)
    logs.log(
        "Added %d tests using Radamsa mutations." % (new_corpus_size - old_corpus_size)
    )


def generate_new_testcase_mutations_using_ml_rnn(
    corpus_directory, new_testcase_mutations_directory, fuzzer_name, generation_timeout
):
    """Generate new testcase mutations using ML RNN model."""
    # No return value for now. Will add later if this is necessary.
    ml_rnn_generator.execute(
        corpus_directory,
        new_testcase_mutations_directory,
        fuzzer_name,
        generation_timeout,
...
def get_testcase_run(stats, fuzzer_command):
    ...
    testcase_run = fuzzer_stats.TestcaseRun(
        None, job, build_revision, current_timestamp()
    )
...
def find_fuzzer_path(build_directory, fuzzer_name):
    ...
    for root, _, files in os.walk(build_directory):
        for filename in files:
            if (
                legacy_name_prefix + filename == fuzzer_name
    ...
    # build directory.
    logs.log_warn(
        "Fuzzer: %s not found in build_directory: %s." % (fuzzer_name, build_directory)
    )
    return None
...
def get_merge_timeout(default_merge_timeout):
    ...
    return get_overridable_timeout(default_merge_timeout, "MERGE_TIMEOUT_OVERRIDE")
...
def get_issue_owners(fuzz_target_path):
    ...
    if environment.is_trusted_host():
        owners_file_path = fuzzer_utils.get_file_from_untrusted_worker(owners_file_path)
    ...
    with open(owners_file_path, "r") as owners_file_handle:
        ...
        for line in owners_file_content.splitlines():
            ...
            if stripped_line == "*":
                ...
                continue
            if stripped_line.startswith("per-file") or stripped_line.startswith(
                "file:"
            ):
...
def get_issue_metadata(fuzz_target_path, extension):
    """Get issue metadata."""
    metadata_file_path = fuzzer_utils.get_supporting_file(fuzz_target_path, extension)
...
def get_seed_corpus_path(fuzz_target_path):
    ...
    if len(archive_paths) > 1:
        logs.log_error(
            "Multiple seed corpuses exist for fuzz target %s: %s."
...
def unpack_seed_corpus_if_needed(
    ...
    logs.log(
        "Unarchiving %d files from seed corpus %s." % (idx, seed_corpus_archive_path)
    )
