...
def _get_authorization(force_reauthorization, configuration):
    ...
    print(
        "Please login at the following URL to authenticate: {oauth_url}".format(
            oauth_url=oauth_url
        )
    )
...
def request(
    url, body=None, method=POST_METHOD, force_reauthorization=False, configuration=None
):
    ...
    if AUTHORIZATION_HEADER in response:
        ...
        utils.write_data_to_file(
            response[AUTHORIZATION_HEADER], AUTHORIZATION_CACHE_FILE
        )
