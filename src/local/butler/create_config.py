...
class DomainVerifier(object):
    ...
    def get_domain_verification_tag(self, domain):
        ...
        response = (
            self.api.webResource()
            .getToken(
                body={
                    "verificationMethod": "FILE",
                    "site": {"identifier": domain, "type": "SITE"},
                }
            )
    ...
    def verify(self, domain):
        ...
        self.api.webResource().insert(
            body={"site": {"identifier": domain, "type": "SITE"}},
            verificationMethod="FILE",
        ).execute(num_retries=_NUM_RETRIES)
    ...
    def add_owner(self, domain, email):
        ...
        response = (
            self.api.webResource().get(id=domain).execute(num_retries=_NUM_RETRIES)
        )

        if email not in response["owners"]:
            response["owners"].append(email)

        self.api.webResource().update(id=domain, body=response).execute(
...
def compute_engine_service_account(gcloud, project_id):
    ...
    return (
        get_numeric_project_id(gcloud, project_id)
...
def execute(args):
    ...
    bucket_replacements = (
        ...
        ("test-fuzzer-logs-bucket", project_bucket(args.project_id, "fuzzer-logs")),
        ("test-corpus-bucket", project_bucket(args.project_id, "corpus")),
        ("test-quarantine-bucket", project_bucket(args.project_id, "quarantine")),
        ("test-shared-corpus-bucket", project_bucket(args.project_id, "shared-corpus")),
    ...
    add_service_account_role(
        gcloud, args.project_id, service_account, "roles/datastore.importExportAdmin"
    )
    add_service_account_role(
        gcloud, args.project_id, service_account, "roles/iam.serviceAccountTokenCreator"
    )
